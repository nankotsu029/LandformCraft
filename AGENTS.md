...
このファイルはLandformCraftを変更する開発エージェント（Codex／Claude Code／Antigravity／Cursor）向けのルート規範である。
...
* `docs/design-v2/model-assignment.md`（Taskごとの実行モデル・Effort・レビュー体制）
...
1. `git status --short`
2. `docs/roadmap.md`（担当Trackの`次のTask`を確認する）
3. `docs/design-v2/task-execution-guide.md`
4. Task Index内の担当Trackの未完了Task 1件
...
Taskは5トラックで管理する。Track A（V2-4→V2-5→V2-6→V2-18→V2-17）、Track B（V2-7→V2-14 画像忠実性）、Track C（V2-8→V2-13 スケール）、Track D（V2-9 地形基盤拡張）、Track E（V2-10→V2-15→V2-16 地形拡張）である。V2-9／V2-10は完了済みで、2026-07-22全地形監査により既存planの公開接続をV2-15、deferred／new terrainとcompositionをV2-16、Paper実機証跡と昇格をV2-17へ分離した。V2-17は`V2-15-47`／`V2-16-19`を前提とし、実機未測定Featureを`SUPPORTED`へしない。2026-07-22 macro foundation監査によりV2-18（macro foundation／intent conformance、12 Task）をTrack Aへ登録し、V2-15のproduction export／placement昇格系Taskは`V2-18-09`完了と対象kindのcomposition role登録を前提とするstage gateに従う（registry／Schema／codec／offline plan／determinism系Taskは継続可）。各回は`docs/roadmap.md`が示す担当Trackの次の未完了`V2-x-NN`を1件だけ実行する。
...
トラックは並行実行できるが、次を厳守する。

* 同一ファイルを複数エージェントが同時編集しない
* `format.v2.release`等の共有領域を変更するTaskは直列にする
* 他TrackのTaskを同じ作業で抱き合わせない
...
実行モデルとEffortはセッション開始時に`docs/design-v2/model-assignment.md`から決め、昇格は失敗ベースのラダーに従う。full suiteはPhase gate Taskだけで実行する。
...
* 規模別実行は`model.v2.scale`／`core.v2.scale`のscale契約（ADR 0016）を正本とし、寸法・tile・予算の新しいhard-codeを追加しない
* 画像からの抽出は`format.v2.constraint.extract`のdraft契約（ADR 0017）を通し、draftを暗黙にhard constraint化しない
...
上記の凍結は恒久ではなく、[ADR 0035](docs/adr/0035-v1-retirement-governance.md) が定める範囲と順序でのみ解除する。解除には次の**両方**が必要である。

* ADR 0035が `Accepted` である（人間承認）
* 対象資産についてADR 0035 D2の前提Taskが完了している

ADR 0035は2026-07-20に`Accepted`となり、`V2-12-05`と`V2-12-06`はそれぞれ独立した人間承認を得て2026-07-21に完了した。V2-12-06はD8 preflightをfail closedで通過し、R1〜R8だけを退役した。以後のTaskはv1資産を削除・改名せず、D2b／D3のlegacy read／verify／migrate、immutable golden、custom asset、version中立基盤を維持する。

解除後も次を守る。

* v1契約の変更権限は完了済み`V2-12-05`／`V2-12-06`だけに限定されていた。後続Taskはlegacy compatibility境界を変更しない
* 削除してよいv1資産はADR 0035 D2aのR1〜R8に限る。一覧外の削除が必要になったら停止しADR amendmentを取得する
* v1 artifactの**read／verify／migrate**経路はADR 0035 D2bのlegacy compatibility境界として維持する。「v1で新しく作れない」ことと「v1資産を読めない」ことを混同しない
* ADR 0035 D3が維持対象とした資産（v1 golden corpus、version中立の共通基盤、custom asset catalog、運用verb、参照用ADR）を削除しない。v1 goldenはR1削除前にimmutable archive＋v2 equivalence testへ変換する
* `V2-12-06`のD8 drain gateはneutral archive＋strict read-backでPASSした。機械可読証跡は`docs/design-v2/audits/v2-12-06-drain-evidence.json`を正本とする
* 改名はADR 0035 D4に従う。Java識別子の`V2` suffix除去は自由、Schema `$id`／schemaファイル名／capability名／artifact type・version／module・stage ID／named seed／contract ID／generator version／error code／metric labelはADRが明示承認した場合だけ変更する
* `V2-12-02`〜`V2-12-04`はv1を変更しないため、ADR 0035の承認を待たずに実行できる
* ADR 0035 D10で維持対象とされたcommand（`asset`／`doctor`／`job`／`recovery`／`version`／`help`）を「v1 commandの削除」に巻き込まない

ADR 0035 D0のとおり、ADR 0035は削除・改名を許可するgovernance境界の正本、`docs/design-v2/task-index.md`は実行ScopeとAcceptance gateの正本である。両者が矛盾する場合はより厳しい制約を適用して停止し、同期するまで削除・改名を実行しない。
...
* dense voxel配列（1000×1000×512等）を正本または常駐配列にしない
* LARGE（1024超〜3072）は全域full解像度working setの常駐を禁止し、coarse大域計画＋streaming tile実行とする（`docs/design-v2/scale-and-streaming.md`）。V2-8の各gate完了まで、LARGEを生成可能・supportedと表現しない
