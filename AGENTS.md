...
Taskは5トラックで管理する。Track A（V2-4→V2-5→V2-6）、Track B（V2-7 画像忠実性）、Track C（V2-8 スケール）、Track D（V2-9 地形基盤拡張）、Track E（V2-10 地形拡張）である。Track Dは`V2-6-01`／`V2-6-02`と`V2-8-01`だけを前提にTrack A／Cと並行でき、Track Eは対応するTrack D Taskを前提とする。Track D／Eは完了済みだが、Paper apply capabilityの`SUPPORTED`化は`V2-6-06`〜`V2-6-10`と`V2-6-19`の完了後に別Taskで接続する。各回は`docs/roadmap.md`が示す担当Trackの次の未完了`V2-x-NN`を1件だけ実行する。
