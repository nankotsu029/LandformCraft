...
V2-15-04では、historic TerrainIntent v2を`LEGACY_V2`として保持したまま、ADR 0036の
46-kind `CANONICAL_V2` Schema／model／strict migrationを並設しました。projectionは必ず明示し、
旧14 kindのseed namespaceを`legacySeedBinding`としてcanonical checksumへ保持します。この契約は
generator／Release capability／Paper supportを追加しません。

V2-15-05では、既存production exportのgenerator／validator／preview／export選択をcompile-time
registryへ分離しました。現在feature routeがあるのは従来どおりcoastal 4 featureと`surface-2_5d`だけで、
未接続featureやpartial handlerはartifact生成前に拒否します。V2-15-06は同じcoastal featureへ
`hydrology-plan` shared artifact overlay（empty-graph routing／reconciliation／validation／preview）の
production Application Serviceを追加しました。V2-15-07はさらに`environment-fields` shared overlay
（geology／climate／snow／material／palette／ecology／validation／preview）を追加し、
`hydrology-plan`＋`surface-2_5d` dependencyを同一Releaseでstrict verifyします。V2-15-08はそのchainへ
bounded SDF／ordinal固定ordered CSG／AABB index／volume validation／3D tileを重ねる
`sparse-volume` production Application Serviceを追加し、既存Release 2 capability prefixをdirectory／ZIPで
strict read-backします。CLI既定exportはsurface-onlyのまま、個別hydrology／environment／volume Featureの
公開配線とSchema／Feature Support Catalog／Paper supportの昇格はありません。V2-15-09は
[ADR 0037](docs/adr/0037-foundation-surface-to-surface-2_5d-mapping.md)に従うfoundation→`surface-2_5d`
export adapterを追加し、V2-15-10は[ADR 0039](docs/adr/0039-offline-production-route-eligibility.md)候補Aに
従いRIVER／MEANDERING_RIVERへ最初のoffline production route（dispatch `v1→v2` contract bump）を配線しました。
V2-15-11は同パターンでLAKE（CLOSED basinのみ、`V2-19-01`のsemantic materialization義務込み）を配線し、
`LakeBedMaterializationV2`がCARVE_SOLID→ADD_FLUIDで基本をblock化しました。oxbow cutoff subtype
（OXBOW_LAKE）は関係graph束縛の別生成経路のため`V2-15-48`へ分離しています。V2-15-12は同パターンで
CANYON（既存MEANDERING_RIVER reachへHARD WITHINで束縛される横断面、fluid非所有）を配線し、
`CanyonBedMaterializationV2`がCARVE_SOLIDのみで基本をblock化しました。着手中に発見した
`HardPreflightGateV2`のOFFLINE_PRODUCTION route宛HARD relation誤検出gapも同Taskで是正しています。
V2-15-13は同パターンでWATERFALLのsurface側（既存MEANDERING_RIVER reachへHARD `ON_PATH_OF`で
束縛される落差のうち、2.5D surfaceが所有する落ち口下流のplunge basin）を配線し、
`WaterfallBasinMaterializationV2`がCARVE_SOLID→ADD_FLUIDでblock化しました。落水柱と滝裏空洞
（WATERFALL_VOLUME overlay）は`sparse-volume` capability prefixを要し、そのprefixが含む
`environment-fields` pipelineが宣言済みriver同梱Blueprintを`V2-19-10`の判断でfail closed拒否する
ため、同leafでは完了できず`V2-15-49`／`V2-15-50`へ分離しています。

2026-07-22の[macro foundation監査](docs/audits/macro-foundation-conformance-audit-2026-07-22.md)を受けて
登録したV2-18（全13 Task完了）は、[ADR 0038](docs/adr/0038-macro-foundation-contract.md)のmacro
foundation契約を実装しました。これにより`surface-2_5d`のexportはHARD `LAND_WATER_MASK`と
`foundationBaseLevels`による明示foundation入力を必須とし（`V2-18-10`以降）、foundation owner被覆が
100%未満のrequestは`v2.export.foundation-owner-coverage-incomplete`でfail closedに拒否されます。
2026-07-23の[横断監査](docs/audits/cross-cutting-audit-2026-07-23.md)を受けて登録したV2-19（全16 Task完了、
Phase gate `V2-19-16`は2026-07-24人間承認で閉鎖）は、公開配線leafへ「final canonical block streamからの非空効果実測」を必須化する
semantic materialization gate（`V2-19-01`）を導入したうえで、reference image付きdesignの修理
（`V2-19-03`）、constraint source→Intent binding authoring（`V2-19-04`）、RIVER／MEANDERING_RIVERの
block materialization（`V2-19-05`）、`HEIGHT_GUIDE`のmacro foundation elevation source化（`V2-19-06`）、
`PLAIN`を最初のfoundation producer kindとして接続（`V2-19-07`）、design-time support lint
（`V2-19-08`）、[ADR 0040](docs/adr/0040-coastal-contributor-set-cardinality.md)によるcoastal 4 kind
必須の緩和（`V2-19-09`、任意の部分集合でexport可能）、閉じたrole catalogによるsurface columnの
block語彙とenvironment materialのblock反映（`V2-19-10`）、[ADR 0041](docs/adr/0041-coherent-detail-kernel.md)
によるmacro foundation背景標高のcoherent detail（`V2-19-12`、任意の`foundation-detail`で平坦なbase levelを
空間的に連続なmulti-scale detailへ置換、振幅hard bound・cell-hash非流用・halo 0でwhole==tiled）、
[ADR 0043](docs/adr/0043-mask-feature-reconcile-pre-pass.md)によるmask⇔feature reconcile pre-pass
（`V2-19-14`、任意の`mask-reconcile`で宣言feature geometry全体をHARD land-water maskへ1回の剛体平行移動で
整列。maskは動かさず既存のfail-closed gateも緩めないため、mask⇔seed束縛によるauthoring負担だけが下がります）を
実装しました。いずれもoffline経路の到達性・
実体化の強化であり、Paper `SUPPORTED`能力を追加するものではありません（`paper_apply`はV2-17専管の
まま）。Feature Support Catalogのsupport列、公開dispatch到達性、block materializationは別軸として
扱われ、詳細は[Current Feature State Machine Registry](docs/design-v2/current-feature-state-machine-registry.md)の
`public-dispatch-reachability-v1` projectionを参照してください。

2026-07-17の再設計（[ADR 0015](docs/adr/0015-adopt-v2-foundation-three-track-roadmap.md)）で、roadmapを3トラック（A: Environment→Sparse volume→Placement、B: 画像忠実性 V2-7、C: スケール V2-8）へ再構築しました。新境界として、通常画像からの決定論的制約抽出（V2-7 core＋`V2-14-01`配線）と、SMALL／MEDIUM／LARGE（最大3072、推奨3000）のscale契約・tile plan・事前admission（`scale-admission-v1`、V2-8-01）を`EXPERIMENTAL`で実装済みです。画像抽出はCLI `extract`／`promote`と`request constraint-map`宣言→`v2 export`まで到達できますが、runtimeは`EXPERIMENTAL`のまま、Paper UIは未接続、`SUPPORTED`昇格は別承認です。scale契約はLARGE生成経路へ未接続で、**3000×3000の生成はまだできません**（request／Schema水平上限はMEDIUM=1024、`V2-13-02`。Paper配置のcatalog上限は1000×1000のまま。1024² offline generationは`V2-13-03`のfollow-upでE2E成立し、1024² FAWE配置も`V2-13-04`／`V2-13-06`で実測済みですが、catalog昇格は別承認です）。Track AはV2-5（Sparse volume）を`V2-5-18`のPhase gateまで完了し、offline `sparse-volume` Releaseを生成・strict verifyできます。V2-6はplacement plan〜Recovery、provider／manual／image v2 design path、Release hardening、運用metricsに加え、strict canonical source（ADR 0033）と明示的なPaper application lifecycle（ADR 0034、V2-6-21）まで実装しました。operation-bound Undoと保守的Recoveryを備えたRelease 2 lifecycleは、V2-12-06以降唯一のproduction placement経路です。WorldEdit 7.3.19実機smoke（`V2-6-14`）とFAWE 2.15.2単独smoke（`V2-6-15`）は完了しました。500／1000 Paper実測Task（`V2-6-16`／`V2-6-17`）は無効化済みです。`V2-6-18`で能力別Feature Support CatalogとPaper寸法hard limitを固定し、未測定寸法とPaper能力列をproduction `SUPPORTED`にしません。`V2-6-19`のRelease candidate auditでV2-6 Phase gateを閉じました（[audit](docs/design-v2/audits/v2-6-phase-gate.md)）。gate自体は能力を昇格せず、続く`V2-11-01`が実機evidenceの範囲だけをcatalog昇格しました。`paper_apply`／`post_apply_validation`／`snapshot`／`rollback`／`restart_recovery`が`SUPPORTED`なのは、smoke実測済み`surface-2_5d` capabilityのSANDY_BEACH／BREAKWATER_HARBOR／HARBOR_BASIN／ROCKY_CAPEを適用する場合だけで、`hydrology-plan`／`environment-fields`／`sparse-volume` featureは`EXPERIMENTAL`、V2-9／V2-10 foundation featureは`UNSUPPORTED`のままです。`V2-11-04`／`V2-11-05`のFAWE 2.15.2実測（500×500／1000×1000）を受けて、`V2-11-06`がcatalogのPaper寸法上限を1000×1000へ昇格しました。V2-13-06はapply sliceを同一FAWEホストで較正し、production既定を1024 mutations/sliceへ変更しました（1000²／1024²でAPPLY約93.4%、lifecycle約74.3%短縮、full verify／Undo／Recovery不変）。この性能変更は寸法や能力の昇格ではありません。この寸法evidenceはFAWE単独で、WorldEdit 7.3.19では64×64のまま、1000超は通常配置で拒否されます。
...
CLIはv2のみです。`lfc <verb>`は恒久的な明示形`lfc v2 <verb>`と同じで、主要コマンドは`request`、`intent`、`design`、`generate`、`export`、`preview`、`extract`、`promote`、`job`、`candidate`、`journal-verify`、`recovery inspect`、`migrate`、`doctor`、`version`、`asset`です。`extract`／`promote`はoperator workstationのimage pathを読むためCLI専用です。`request constraint-source`は`LAND_WATER_MASK`／`HEIGHT_GUIDE`／`ZONE_LABEL_MAP`共通のsource宣言、`intent bind`／`intent bindings`（CLI専用）はその宣言からTerrainIntent側のconstraint bindingを生成・確認します（`V2-19-04`）。共通optionは`--data-dir`、`--json`、`--quiet`、`--verbose`です。
...
キーは `OPENAI_API_KEY`／`ANTHROPIC_API_KEY`など、`config.yml`で指定した環境変数からだけ取得します。model IDはcommandまたは空でないdefault modelとして明示します。v2のreference image roleは `MOOD_REFERENCE`、`TOP_DOWN_SKETCH`、`MATERIAL_REFERENCE`、`STRUCTURE_REFERENCE`、`OBLIQUE_TERRAIN_REFERENCE`、`MULTI_VIEW_REFERENCE` の6種です（AI提案入力専用。座標constraintやheight／zoneの決定論的入力はconstraint map経路）。既定の`design`はTerrainIntent v2向けのcapability明示dispatch（OpenAI／Anthropic／manual／soft draft、no-fallback）を使います。詳細は [AI Provider](docs/ai-providers.md) のV2-6-11節とADR 0029、[Direct Constraint Maps v2](docs/design-v2/image-constraint-maps.md)、[User Guide](docs/user-guide.md)を参照してください。
...
- [全地形カタログ監査 2026-07-22（現行inventory）](docs/audits/terrain-catalog-full-audit-2026-07-22.md)
- [地形Feature Gap監査 2026-07-18（履歴）](docs/audits/terrain-feature-gap-audit-2026-07-18.md)
