...
公開配線とSchema／Feature Support Catalog／Paper supportの昇格はありません。
...
CLIはv2のみです。`lfc <verb>`は恒久的な明示形`lfc v2 <verb>`と同じで、主要コマンドは`request`、`design`、`generate`、`export`、`preview`、`extract`、`promote`、`job`、`candidate`、`journal-verify`、`recovery inspect`、`migrate`、`doctor`、`version`、`asset`です。`extract`／`promote`はoperator workstationのimage pathを読むためCLI専用です。共通optionは`--data-dir`、`--json`、`--quiet`、`--verbose`です。
