...
- reference image soft draft は SOFT 提案だけ。HARD `mapReferences` への暗黙昇格は禁止（ADR 0017）。secure ファイル封筒（`SecureImageExtractionEnvelopeV2`）は実装済みだが Request／CLI／Paper 未接続
