...
### Additive note (V2-13-01, 2026-07-21)

`OperationalMetricsSnapshotV2` の closed label 集合へ、placement lifecycle の wall-clock stage
duration 6 label（`PLACEMENT_STAGE_DURATION_{PLAN,SNAPSHOT,APPLY,SETTLE,VERIFY,UNDO}`）と unit
`SECONDS` を **additive** に追加する。これらは runtime collector（`OperationalMetricsCollectorV2`）
が出力する固定 17 label 集合（`RUNTIME_LABELS`）には含めず、offline measurement runner だけが
populate する。よって既存 runtime snapshot の sample 集合と canonical checksum は不変であり、schema
`$id`・`contractVersion`（`operational-metrics-snapshot-v1`）も変更しない。目的は V2-11-05 の総
wall 時間（~106 min/pass）を工程別へ分解し、最大ボトルネック工程を committed evidence から特定する
ことに限る（catalog 昇格・safety 順序・verify 範囲は変更しない）。
