...
- [0036: canonical Feature identifierの処置と互換移行を固定する](0036-canonical-feature-identifier-disposition.md) — Accepted
- [0037: V2-9／V2-10 foundation 2.5D surfaceを既存`surface-2_5d`へ写像する](0037-foundation-surface-to-surface-2_5d-mapping.md) — Accepted
- [0038: 全surface XZ cellへeffective foundation ownerを要求するmacro foundation契約](0038-macro-foundation-contract.md) — Accepted
- [0039: offline production route eligibility（Paper非昇格の公開 export／CLI 配線）](0039-offline-production-route-eligibility.md) — Accepted（2026-07-23、候補A: dispatch `v1→v2` 採択）
- [0040: coastal contributor set cardinality（`surface-2_5d` modifier tierの被覆要求撤廃）](0040-coastal-contributor-set-cardinality.md) — Accepted（2026-07-24、D1: contributor集合はcoastal 4種の任意の部分集合（サイズ0〜4）採択）
- [0041: coherent detail kernel（macro foundation背景標高のbounded multi-scale detail）](0041-coherent-detail-kernel.md) — Accepted（2026-07-24、D2: background base-level cellのみ／D3: request level optional採択）
- [0042: Blueprint typed plan envelope（`WorldBlueprintV2`拡張面積の内部集約とexecutableKinds単一ソース化）](0042-blueprint-typed-plan-envelope.md) — Proposed（2026-07-24、D1 内部registry集約／D3 executableKinds単一ソース化を提案、wire形変更のD2は将来ADRへ分離）
- [0043: mask⇔feature SOFT reconcile pre-pass（宣言geometryのmaskへの決定論的整列）](0043-mask-feature-reconcile-pre-pass.md) — Accepted（2026-07-24、D1: 宣言feature集合全体への単一剛体平行移動／D3: request level optional `maskFeatureReconcile`（tolerance 1..8、opt-in）採択）
