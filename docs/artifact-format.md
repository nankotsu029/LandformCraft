...
`V2-15-09`はV2-9／V2-10 foundation 2.5D surfaceを同じ`surface-2_5d` exact setへ写像するoffline adapterを追加した（[ADR 0037](adr/0037-foundation-surface-to-surface-2_5d-mapping.md)）。`FoundationSurfaceExportAdapterV2`がplain／hill mergeからland-water／heightを投影し、coastal planが無いBlueprintではoverlay previewを空値のまま`CoastalValidatorV2`／固定11 preview／tileへ載せ、`Release2FoundationSurfaceExportApplicationServiceV2`が既存`ReleaseSurfacePublisherV2`へ渡す。`ProductionDispatchRegistryV2`の`["surface-2_5d"]`第二pipelineには登録しない。fixtureは`examples/v2/foundation/plain-hill-64.*`、Acceptanceは`Release2FoundationSurfaceExportApplicationServiceV2Test`である。Feature昇格とCLI既定切替は含まない。
