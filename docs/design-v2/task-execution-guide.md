...
`V2-6-16`（500×500）／`V2-6-17`（1000×1000）実world計測は無効化（CANCELLED）である。再実測が必要なら新Task IDを追加し、未測定寸法をcatalog `SUPPORTED`へ上げない。再実測Taskは`V2-11-04`（500）／`V2-11-05`（1000）として登録済みで、専用高memory実測ホスト、Gradleから分離した単独Paper JVM（いずれも`run-fawe/`＋FAWE 2.15.2単独）、RCON、server PID単独のheap／RSS／GC計測、`V2-11-02` measurement profileを前提とする。
