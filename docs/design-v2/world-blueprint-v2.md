...
> Status: V2-0〜V2-3のPhase gateに加え、V2-4-01〜03の`geologyPlan`／`lithologyPlan`／`strataPlan`と、V2-4-04の`climatePlan`、V2-4-05の`waterConditionPlan`（wetness／salinity／hydroperiod／groundwater／tidal influence）を実装した。environmentは`EXPERIMENTAL`で、snow／material連携は未実装である。いずれもv1から分離され、現行 [WorldBlueprint v1](../../src/main/java/com/github/nankotsu029/landformcraft/model/WorldBlueprint.java) は変更していない。次は`V2-4-06`である。
...
EcologyPlan: habitat、assemblage、density、placement rule
