...
> Status: V2-3／V2-4 Phase gate完了。V2-4のgeology／lithology／strata、coarse climate runoff priorへの明示version transition、regional wetness／salinity／hydroperiod、mangrove／coral reconciliation hookを統合した。Hydrology plan、fixed prior、既存routing artifactのversion／値／checksumは変更していない。`V2-5-08`／`V2-5-09`で局所`UNDERGROUND_LAKE`と静的`SEA_CAVE` marine fluidを、`V2-5-13`でwaterfall volume（falling column／behind-fall／plunge pool、checksum-bound）を追加し、V2-5 Phase gate（`V2-5-18`、[Volume監査](audits/v2-5-phase-gate.md)）でWATERFALLを含むvolume経路をoffline `SUPPORTED`へ昇格した。dynamic tide／Paper経路と現行v1 river/lake実装は未変更である。Track Aの次は`V2-6-01`である。
...
`SPRING` nodes are owned by the V2-10-10 surface `SPRING` foundation slice (`SpringPlanV2` binds a general `RIVER` SOURCE+HEADWATER and emits `hydro.spring.<featureId>`). This is distinct from V2-10-03 `KARST_SPRING` karst graph discharge.

Reach-cutoff oxbow basins are owned by the V2-10-11 `OXBOW_LAKE` foundation slice (`OxbowLakePlanV2` binds a parent `RIVER`／`MEANDERING_RIVER` reach with FIXED `CLOSED` stagnant level and wetland handoff). This is distinct from open-spill `LAKE` (`LakePlanV2`) and deferred barrier basins (`DAM_RESERVOIR`).
...
V2-9-13では一般foundation `RiverPlanV2`へgraph-internal roleを拡張した。`HEADWATER`／`STREAM`／`TRIBUTARY`／`CONFLUENCE`／`DISTRIBUTARY`／`RAPIDS`および`SANDBAR`／`RIVER_ISLAND`／`PLUNGE_POOL` childはFeatureKindではなく、node／reach／child semanticとして同じgeneral IRへ載せる。`WATERFALL_CHAIN`は複数WATERFALL node＋plunge-pool childの`COMPOSITE_PRESET`（`WaterfallChainPlanV2`）であり、専用waterfall-chain world generatorではない。split／mergeの整数discharge share総和、elevation continuity、modifierの安定順、node／edge／branch budget、cycle／orphanをstrictに拒否する。既存`MEANDERING_RIVER`／`DELTA`／`WATERFALL` hydrology fixturesのID／checksumは変更しない。
...
V2-4-09 mangrove wiringはscan orderを`kind-feature-constraint-v2`へ上げ、`MANGROVE_TIDAL_LINK`をStage 6 verify-only unary constraintとして追加する。`MangroveWetlandPlanV2.tidalNetworkHook`が非nullのとき、reconciliation planは`mangrove.<featureId>.tidal-link` variable（`MARINE_CONNECTION` baseline 1）と対応する`MANGROVE_TIDAL_LINK` constraintをtidal connectionの後、fjord connectionの前にfreezeする。hookなし mangrove planはreconciliationへ寄与しない。

V2-4-10 coral reef wiringはscan orderを`kind-feature-constraint-v3`へ上げ、`REEF_LAGOON_PASS`をStage 6 verify-only unary constraintとして追加する。`CoralReefPlanV2.passHooks`が非空のとき、reconciliation planは`reef.<featureId>.lagoon-pass` variable（`MARINE_CONNECTION` baseline 1）と対応する`REEF_LAGOON_PASS` constraintをmangrove tidal linkの後、fjord connectionの前にfreezeする。pass hookなし reef planはreconciliationへ寄与しない（sealed lagoonはgenerator metricで検出する）。

`BoundedHydrologyReconcilerV2`はinteger millionthsだけを使い、plan constructorが固定したkind／feature／constraint順で必ず3 pass走査する。reach、lake spill、waterfall baseは右辺だけを最大16 blockまで補正し、delta／tidal／mangrove tidal link／reef lagoon pass／fjord connectionはverify-onlyである。結果はfinal scalar state、constraintごとのactual delta／residual／correction count、`NONE`／`HARD_CONFLICT`／`ADJUSTMENT_LIMIT`／`UNRECOVERABLE_CONNECTION`／`NON_CONVERGENCE`、resource usage、state／result／canonical checksumを持つstrict artifactとなる。recoverable／unrecoverable、hard lock、補正上限、3 pass non-convergence、cancel、Schema／checksum改変を検査し、feature／candidate入力順、thread数、locale、timezoneでresidualとchecksumが一致する。full field validator／preview、`hydrology-plan` Release capability、Paper配置、V2-4 fieldは含めていない。
...
### 14.17 V2-4-15 environment integration

`EnvironmentPhaseGateV2Test`はsnowy mountains、mangrove、coral、volcanic、canyon／waterfall scenarioでV2-3 Hydrology checksum bindingを保持したまま、snow／material／palette／ecology／feature-material plansを統合した。V2-4-15完了によりALPINE／GLACIAL mountain、MANGROVE_WETLAND、CORAL_REEF、VOLCANIC_ARCHIPELAGOと`environment-fields`はoffline `SUPPORTED`である。WATERFALLはfalling-water／behind-fall volumeを欠くため`EXPERIMENTAL`を維持する。

### 14.18 V2-19-05 river block materialization

2026-07-23横断監査は、`V2-15-10`が接続した`RIVER`／`MEANDERING_RIVER`のoffline production
routeがrouting／reconciliation／validation artifactを作るだけで、公開Releaseのfinal canonical
block streamを1 blockも変えない（同じcoastal契約のReleaseとtile byte一致）ことを確定した。
`V2-19-05`はこの経路をblock実体化へ接続する。

`RiverBedMaterializationV2`（`river-bed-materialization-v1`、`core.v2.export`）は、routing解と
reconciliation artifactが揃った後・**最初のtileを書く前**に、宣言reachのbounded river bed field
（channel／bank AABBのみ。全domainを確保しない）をfreezeする。適用は列ごとに次の順で、
`volumetric-terrain.md`のordered CSG doctrineと同じ責務分離を守る。

1. `CARVE_SOLID` — `[bedY+1, surfaceY]`をvoid化する。solid massだけを取り除き、fluidを作らない。
2. `ADD_FLUID` — そのvoidの内側`[bedY+1, waterSurfaceY]`へwaterを置く。fluidの所有はこちら側だけで、
   solidを置換しない（置換が必要になるroute＝water surfaceがcarve envelopeを超えるrouteは拒否）。

bed block自身とその下はcoastal surface resolverの所有のままで、riverはbed材質の再ライニングを行わない
（実測effect classは`SOLID_SHAPE`＋`FLUID`のみ、`MATERIAL`は空）。

宣言interaction（`RiverBedMaterializationV2.InteractionV2`）と対応するfail-closed rule idは次のとおりで、
推測補完も無音修復も行わない。

| Interaction | 内容 | rule id |
|---|---|---|
| `SOURCE_TERMINUS` | 実体化するbedはreconcile済みbedであること（plan値との乖離は拒否） | `v2.river.route-not-frozen` |
| `MOUTH_TERMINUS` | v1のmouthは内陸終端。海へのmouth junctionはdelta／estuary leafの責務 | `v2.river.marine-contact` |
| `BANK_ENVELOPE` | channel外周（bank／floodplain）はcarve／fill対象外で、water surface以上に立つこと | `v2.river.leak-envelope` |
| `COASTAL_FOUNDATION` | channelはmacro foundation背景のみを掘る。coastal modifier所有cellとの重なりは拒否 | `v2.river.coastal-owner-conflict` |
| `MACRO_MEDIUM` | land-water mediumはHARD mask所有のまま（ADR 0038）。riverはblockだけを変える | `v2.river.marine-contact` |

これに加えてvertical bounds（`v2.river.vertical-bounds`、bedはbedrock床より上・carve topはmaxY以下）と
bounded footprint budget（`v2.river.materialization-budget`、MEDIUM上限1024²cell、AABB境界へ
channelが達したら拒否）を持つ。river はland-water fieldを書かないため、macro foundation owner gate、
EDGE評価、coastal conformance測定は影響を受けない。

検証は公開Releaseのfinal tile streamからのみ行う（`RiverBlockConformanceV2`、test scope）。
bed depth（各channel列がbed＋宣言water depth＋上方は空）、water continuity（XZ 4連結で単一water body。
段差bedのため3D面連結は2.5D reachの正しい問いではない）、source→mouth reachability、
leak envelope（channel waterがchannel外のairへ接しない・外周列がwater surface高でsolid）を測る。
plan-only metric（`hydrology/validation.json`）はportfolioの別欄のままで、block実体化の代替にならない。
