...
| E | V2-15 Canonical catalog／existing-generator wiring | 50 | `V2-15-01` | `V2-15-47` |
...
## 15. V2-15 Canonical catalog and existing-generator public wiring（Track E続編、50 Task。`V2-15-11`がoxbow cutoff subtypeを`V2-15-48`へ分離して47→48、`V2-15-13`が`WATERFALL_VOLUME` overlayとその前提であるenvironment stack実測化を`V2-15-49`／`V2-15-50`へ分離して48→50）
...
| `V2-15-11` | 03,06,`V2-19-01` | `LAKE`＋oxbow cutoff subtype public wiring | dam reservoirはV2-16。**stage gateは`V2-19-01`の2026-07-23完了（人間承認済み）で解除された。本leaf以降の公開配線leafは§15.1のsemantic materialization義務（block effect class宣言＋final block streamからの非空効果・形状conformance portfolio case）をAcceptanceへ含む**。**完了（2026-07-24）。Scopeは`LAKE`基本（既存`HydrologyLakeModuleV2`／`LakePlanCompilerV2`／`LakeGeneratorV2`はV2-3-04起源でdedicated binding・export SUPPORTED済みだった）に限定した。`OXBOW_LAKE`（oxbow cutoff subtype）は既存`OxbowLakePlanCompilerV2`／`OxbowLakeGeneratorV2`（V2-10起源）がHARD `SUPPORTED_BY`host＋HARD `ORIGINATES_AT`parent riverの関係graph束縛を要求する構造的に別の生成経路（LAKEのring／spill生成kernelを共有しない）であり、task-execution-guide.md §4の「1 Taskで2個目の独立generatorを抱えない」規約に従い新ID`V2-15-48`へ分離した（隠蔽せず本行に明示pin）。実装: `ProductionDispatchRegistryV2`の`OFFLINE_PRODUCTION`allowlistへ`LAKE`を追加（RIVER／MEANDERING_RIVERと同じ`hydrology-plan`共有pipeline）、`CompositionProfileRegistryV2`のNORMATIVEへ`LAKE`確信度のみ追加（7→8／PROVISIONAL 53→52）、`HydrologyPlanExportPipelineV2`の`requireSharedHydrologyOnly`が`lakePlans()`を許可するよう是正しRIVER／LAKE両kindの共有field sampler（`hydrology.lake.*`と`hydrology.river.*`、共有IR field 4種はkind非依存でmerge）を追加。新設`LakeBedMaterializationV2`（`lake-bed-materialization-v1`、`core.v2.export`）がCLOSED terminal policyの基本だけを`CARVE_SOLID`→`ADD_FLUID`で実体化し（宣言interaction: `SPILL_TERMINUS`／`RIM_ENVELOPE`／`COASTAL_FOUNDATION`／`MACRO_MEDIUM`）、OPEN_SPILLは`v2.lake.spill-not-wired`でfail-closed拒否する（RIVERのmarine mouth「まだ配線されていない」と同じ姿勢。dam reservoirは引き続きV2-16のScope）。`HydrologyPlanExportPipelineV2`のtile書出し前global freezeがriver bedとlake bedを両方適用できるよう合成overlayへ拡張（river→lakeの固定順、どちらか一方のみのrunは既存経路とbyte一致）。intent-conformance portfolioへ`harbor-cove-64-honored-lake`caseを追加し、公開Releaseの`hydrology/validation.json`から`hydrology.lake.basin-cells`metricを読み戻して適合を確認、final canonical block streamからbaseline `harbor-cove-64-honored`比の非空block diff（宣言`{SOLID_SHAPE, FLUID}`と実測完全一致、`MATERIAL`0）を`FeatureMaterializationV2.requireMaterialized`で固定、`LakeBlockConformanceV2`が basin depth／water continuity／leak envelopeをfinal tile streamだけから測定。`public-dispatch-reachability-v1`の`LAKE`を`NOT_PUBLICLY_DISPATCHABLE`→`OFFLINE_PRODUCTION`＋`MATERIALIZED`へ更新（[registry](current-feature-state-machine-registry.md)のCI検査済みprojection更新）。`InputIntegrityPhaseGateV2Test`／`DesignSupportLintServiceV2Test`／`MacroFoundationPhaseGateV2Test`のrestart-state pin（NORMATIVE 7・LAKE PROVISIONAL・offline kind集合）を新状態へ同一commitで更新。generator field math・v1 golden・coastal既存Release checksum・Release format不変、full `./gradlew test`／`./gradlew build` PASS |
| `V2-15-48` | 11 | `OXBOW_LAKE` cutoff subtype public wiring（materialization義務込み） | `V2-15-11`から分離。既存`OxbowLakePlanCompilerV2`／`OxbowLakeGeneratorV2`（V2-10起源、foundation-tier）を再利用し、HARD `SUPPORTED_BY`host surface＋HARD `ORIGINATES_AT`parent river（`RIVER`／`MEANDERING_RIVER`）の関係graph解決を`DiagnosticBlueprintCompilerV2`へ接続、dedicated module登録、`OFFLINE_PRODUCTION`route、block materialization（host／parent river既存bedとの相互作用宣言を含む）、intent-conformance portfolio case追加が必要。dam reservoirはV2-16、2個目の独立generatorをこれ以上抱えない |
| `V2-15-12` | 06 | `CANYON` public CLI／Release wiring | submarine canyon非Scope。**完了（2026-07-24）**。既存`LandformCanyonModuleV2`／`CanyonPlanCompilerV2`／`CanyonGeneratorV2`（V2-3-05起源でdedicated binding・export SUPPORTED・`DiagnosticBlueprintCompilerV2`での既存compile済み）を再利用し、`ProductionDispatchRegistryV2`の`OFFLINE_PRODUCTION`allowlistへ`CANYON`を追加（RIVER／LAKEと同じ`hydrology-plan`共有pipeline）。`CompositionProfileRegistryV2`のNORMATIVEへ`CANYON`確信度のみ追加（8→9／PROVISIONAL 52→51）。`HydrologyPlanExportPipelineV2`の`requireSharedHydrologyOnly`が`canyonPlans()`を許可するよう是正し、river／lake／canyon 3種の共有field sampler（canyonは`hydrology.bed.elevation`のみriver/lakeと共有、fluid系3 fieldは共有しない）を追加。新設`CanyonBedMaterializationV2`（`canyon-bed-materialization-v1`、`core.v2.export`）がCARVE_SOLIDのみ（fluid非所有）でwall／floor／rim／terrace横断面を実体化し、共有channel列は既存`RiverBedMaterializationV2`のCARVE_SOLID→ADD_FLUIDが優先するよう合成順序をcanyon→river→lakeへ拡張した（river overlayがcanyonの上に被さり、共有channel列はriverの判定が勝つ）。CANYONは独立地形ではなく既存`MEANDERING_RIVER`reachへHARD `WITHIN`で束縛される横断面のため、着手中に`HardPreflightGateV2`（V2-18-03起源）の「production-connected」判定が`DiagnosticGateContractV2.isProductionConnected`（coastal 4限定のPaper完全接続axis）だけを見ており、ADR 0039の`OFFLINE_PRODUCTION`route（RIVER／LAKE／CANYON自身を含む）を宛先とするHARD relationを`v2.preflight.hard-relation-unconsumed`で誤って拒否する潜在的gapを発見・是正した（`HardPreflightGateV2`が`PublicDispatchReachabilityV2`も参照するよう拡張。`DiagnosticGateContractV2.isProductionConnected`自体の意味・戻り値・他箇所の利用・pinned testは不変。既存fixtureの`backshore-adjoins-beach`等のcontract-only拒否は不変のまま回帰PASS）。intent-conformance portfolioへ`harbor-cove-64-honored-canyon`case（`MEANDERING_RIVER`reach＋HARD WITHIN`CANYON`、companion riverなしでは`CANYON`を宣言できないためcase自体がRIVERを同梱）を追加し、`hydrology.canyon.floor-cells`／`rim-cells`（新設`HydrologyValidatorV2.measureCanyon`）を読み戻して適合を確認、final canonical block streamからbaseline `harbor-cove-64-honored`比の非空block diff（宣言effect class`{SOLID_SHAPE, FLUID}`、FLUIDは同梱riverの寄与でMATERIAL 0）を`FeatureMaterializationV2.requireMaterialized`で固定、`CanyonBlockConformanceV2`がfinal tile streamだけからcanyon自身の彫り込みがdry（fluid非所有）で共有floor帯がriver bedへ正しく到達することを構造測定。`public-dispatch-reachability-v1`の`CANYON`を`NOT_PUBLICLY_DISPATCHABLE`→`OFFLINE_PRODUCTION`＋`MATERIALIZED`へ更新（[registry](current-feature-state-machine-registry.md)のCI検査済みprojection更新）。`InputIntegrityPhaseGateV2Test`／`MacroFoundationPhaseGateV2Test`／`DesignSupportLintServiceV2Test`のrestart-state pin（NORMATIVE 8・offline kind集合・gated case数）を新状態へ同一commitで更新。generator field math・v1 golden・coastal既存Release checksum・Release format不変、full `./gradlew test`／`./gradlew build` PASS |
| `V2-15-13` | 06,08 | `WATERFALL` public wiring（`WATERFALL_VOLUME` overlayは`V2-15-50`へ分離） | volumeをFeatureKind化しない。**surface側は完了（2026-07-24）／volume側はBLOCKED（新ID分離）**。既存`HydrologyWaterfallModuleV2`／`WaterfallPlanCompilerV2`／`WaterfallGeneratorV2`（V2-3-06起源でdedicated binding・export SUPPORTED・`DiagnosticBlueprintCompilerV2`での既存compile済み）を再利用し、`ProductionDispatchRegistryV2`の`OFFLINE_PRODUCTION`allowlistへ`WATERFALL`を追加（RIVER／LAKE／CANYONと同じ`hydrology-plan`共有pipeline）。`CompositionProfileRegistryV2`のNORMATIVEへ`WATERFALL`確信度のみ追加（9→10／PROVISIONAL 51→50。profile fieldは不変で、STANDALONE＋SURFACE_MODIFICATION／VOLUME_OPERATION／FLUID_OPERATIONのまま）。`HydrologyPlanExportPipelineV2`の`requireSharedHydrologyOnly`が`waterfallPlans()`を許可するよう是正し（graph-levelの`hydrologyPlan().fallPlans()`は引き続き拒否）、river／lake／canyon／waterfall 4種の共有field sampler（waterfallは`hydrology.bed.elevation`のみ共有、fluid系3 fieldは共有しない）を追加。新設`WaterfallBasinMaterializationV2`（`waterfall-basin-materialization-v1`、`core.v2.export`）が**2.5D surfaceの所有範囲＝落ち口下流のplunge basinだけ**をCARVE_SOLID→ADD_FLUIDで実体化する。basin footprintは(1) host riverのfrozen channel cellを構成上すべて除外（宣言interaction`HOST_CHANNEL`）、(2) lip crestの下流半平面へclip（`CREST_HALF_PLANE`）するため、host channelのbed／water決定と衝突せず、composition順序はcanyon→river→lake→waterfall（waterfallは最外だが他overlayが決めたcellを一切要求しない）。containmentは`BASIN_ENVELOPE`／`COASTAL_FOUNDATION`／`MACRO_MEDIUM`でfail-closed。intent-conformance portfolioへ`harbor-cove-64-honored-waterfall`case（`MEANDERING_RIVER` reach＋HARD `ON_PATH_OF` `WATERFALL`、host riverなしでは`WATERFALL`を宣言できないためcase自体がriverを同梱）を追加し、既存`HydrologyValidatorV2.measureWaterfall`の`hydrology.waterfall.drop-millionths`をplan-only列として読み戻し、final canonical block streamからbaseline `harbor-cove-64-honored`比の非空block diff（宣言effect class`{SOLID_SHAPE, FLUID}`と実測完全一致、`MATERIAL` 0）を`FeatureMaterializationV2.requireMaterialized`で固定、`WaterfallBlockConformanceV2`がfinal tile streamだけからbasin深さ・host reachとの水continuity（basin単体2成分／host込み1成分）・**実測落差＝宣言drop**・leak envelopeを構造測定。`public-dispatch-reachability-v1`の`WATERFALL`を`NOT_PUBLICLY_DISPATCHABLE`→`OFFLINE_PRODUCTION`＋`MATERIALIZED`へ更新（[registry](current-feature-state-machine-registry.md)のCI検査済みprojection更新）。generator field math・v1 golden・coastal既存Release checksum・Release format・Paper能力不変。**`WATERFALL_VOLUME`（falling column `ADD_FLUID`＋behind-fall `CARVE_SOLID`、既存`WaterfallVolumePlanCompilerV2`／`waterfall-volume-contract-v1`）は本leafで完了できない**: 当該overlayは`sparse-volume` capability prefixを要し、そのprefixは`environment-fields`を含むが、`EnvironmentFieldsExportPipelineV2`は`V2-19-10`の明示判断で`meanderingRiverPlans`を持つBlueprintをfail-closedで拒否する（environment field stackがriver proximity／freshwater discharge入力を実測できないため、coast-onlyのwater conditionを宣言済みriverの傍らで黙って報告しない）。WATERFALLはhost riverなしに宣言できないため当該拒否に必ず該当する。解除にはenvironment field stackの実測化という独立subsystemが必要で、task-execution-guide.md §4に反するため`V2-15-49`／`V2-15-50`へ分離した（隠蔽せず本行に明示pin） |
| `V2-15-49` | 07 | environment field stackのhydrology proximity実測化（`EnvironmentFieldsExportPipelineV2`のriver／canyon一律拒否解除） | `V2-19-10`が明示的に後続へ委ねた項目。`CoastalEnvironmentFieldStackV2`のriver／lake距離・freshwater discharge・feature material class入力を宣言済みplanから実測し（現在は「そんなFeatureは宣言されていない」値で固定）、`requireSharedEnvironmentOnly`の`meanderingRiverPlans`／`canyonPlans`拒否を解除する。material class→block state経路を通るためenvironment系Releaseのblock streamが変化する点をAcceptanceで明示すること。coastal既存Release（river非同梱）のchecksumは不変であること。`sparse-volume`能力・Feature昇格・Paper能力は非Scope |
| `V2-15-50` | 08,13,49 | `WATERFALL_VOLUME` sparse-volume overlay wiring | `V2-15-13`から分離。既存`WaterfallVolumePlanCompilerV2`／`WaterfallVolumePlanV2`（V2-5-13起源、`waterfall-volume-contract-v1`、VOLUME_OVERLAY）を再利用し、`WaterfallPlanV2`のlip／base anchorとgeometry checksumから`BOUND_TO_FALL`束縛のSDF／CSG planを導出、`SparseVolumeExportPipelineV2`のidentity plan（意図的no-op）を宣言時のみ置換する。falling column `ADD_FLUID`＋behind-fall `CARVE_SOLID`＋plunge pool fluid continuityのblock materialization証跡（`V2-19-01`義務。2.5Dでは作れない「宙に浮く水柱」と「滝裏の空洞」を構造測定すること）とsparse-volume baseline caseとの差分portfolio caseが必要。**Release exact file setを増やさない**（waterfall volume planは工程内plan、公開artifactは既存`sparse-volume`集合のまま）。`WATERFALL_VOLUME`をFeatureKind化しない |
...
| `V2-15-47` | 10..46,48..50 | Phase gate、既存20＋全leaf E2E、full clean baseline、**intent-conformance再検証（`V2-18-12`追記、2026-07-23人間承認）**: 公開接続した各kindがintent-conformance portfolio case（`integration.v2.conformance`）を持ち、portfolio全case適合（登録済み非適合ゼロ＝宣言arm等の宣言集合と実測接続集合の一致、HARD target充足）であること | 親Phaseを閉じる唯一のTask。欠陥は隠さず新IDへ分離 |
...
| `V2-19-08` | — | Design-time support lint（report-only先行）。provider呼出し前のreachable kind／capability集合提示、provider出力後のdispatch dry-run、design auditへの`NON_GATING`記録とCLI／Paper designサマリ表示 | **完了（2026-07-24）**。historic 60-kind Schemaは狭めず、fail-closed化もしていない — いずれも充足。`DesignSupportLintServiceV2`（`core.v2.design`、contract `design-support-lint-v1`）が (1) provider呼出し**前**に`ProductionDispatchRegistryV2`＋`PublicDispatchReachabilityV2`から`DesignSupportSurfaceV2`（production-connected 4／offline-production 3／contract-only 1／required companion 4／design capability、registry checksum＋reachability projection checksum付き）を投影して`TerrainDesignRequestV2`へ載せ、HTTP provider（OpenAI／Anthropic）が独立user sectionとして送る（**出所はdispatch registryでありFeature Support Catalogではない** — 監査§2.2。`export == SUPPORTED`だがrouteが無いkindをreachableと提示しないことをtestで固定）、(2) provider応答**後・publish前**に登録済みcapability setごとの`select`をdry-run（純粋lookup。拒否は例外ではなく答えとして扱う）し、`v2.design.kind-not-publicly-dispatchable`／`kind-contract-only`／`kind-plan-only`／`route-companion-missing`／`dispatch-unselectable`を`NON_GATING`で分類する。runtime preconditionは新設public `ProductionRoutePreconditionsV2`（`CoastalGeneratorRuntimeV2.requiredKinds()`が唯一の出所。coastal 4種必須の緩和は`V2-19-09`）が公開表明し、dispatch選択の外側にあった制約をdesign時点で可視化した。記録先は`audit-v2.json`の**任意**`supportLint`（schemaは`gateClass`を`NON_GATING` constに固定。`DesignAuditV2`のconstructorも`GATING`を拒否するため、fail-closed化はcode・schema・人間承認を同時に要する）で、absentでは書き出さないためv1→v2 migration bundleと既存published packageのbyteは不変。CLI `v2 design`とPaper `/lfc v2 design`は`V2WorkflowServiceV2.summarizeSupportLint`という単一計算を表示する。実測: `PLAIN`単体宣言（`oblique-multi-view`）はrouted kindでありながらcoastal runtimeがcoastal 4種を要求するため`NOT_SELECTABLE`＋companion不足として**publishされたうえで**報告され（従来は`v2 export`まで判らなかった）、`azure-coast`は4 pipeline全て選択可・contract-only 1件、`harbor-cove-64-honored-river`は`hydrology-plan`のみ。`TerrainIntentPromptV2.VERSION`（凍結guard contract）は不変で、advisoryは`design-support-lint-v1`＋reachability checksumで一意識別する。dispatch route集合・registry checksum・`public-dispatch-reachability-v1` projection・sealed catalog・Release format／capability・terrain semantic checksum・v1 goldenは不変。full `./gradlew test`／`./gradlew build` PASS。証跡は [V2-19-08 audit](audits/v2-19-08-design-support-lint.md) |
| `V2-19-09` | 01 | Coastal 4種必須の緩和ADR＋実装。macro foundationが背景ownerを持つ現在、`CoastalGeneratorRuntimeV2.REQUIRED`の全4種要求（V2-2期の残存制約）を見直し、部分集合（beach単体等）の許可条件・checksum影響・conformance caseを定義 | **完了（2026-07-24、人間承認済み）**。ADR＋人間承認必須、既存4種fixtureのchecksum取り扱いはADRで宣言、承認前に実装しない — いずれも充足。[ADR 0040](../adr/0040-coastal-contributor-set-cardinality.md)を起草し、同日の人間承認で**D1「coastal 4 kindの任意の部分集合（サイズ0〜4）」を明示採択**したうえで実装した。根拠は新規の判断ではなく既決契約との不整合の解消である: ADR 0038 **D5-3**が「surface modifierはfoundation ownerではない。被覆要求・単一owner要求を課さない」と明文で禁じている一方、`CoastalGeneratorRuntimeV2.REQUIRED`はまさにmodifier tierの被覆要求として残っていた（[横断監査](../audits/cross-cutting-audit-2026-07-23.md) §2.3の実測「beachだけの海岸はpublic surface exportできない」＝提案T-C1）。地表全域の被覆は`V2-18-10`の`SurfaceFoundationOwnerGateV2`が単独で保証するため、**D2で無条件に緩和**した（legacy baseline経路の部分集合は「4種不足」ではなく`v2.export.foundation-owner-coverage-incomplete`で拒否され、拒否されるという結論は不変）。**D3**により欠落kindのdescriptor field 9本は「footprintが空のcontributor」と同一のcanonical OUTSIDE値（beach width＝`NO_DATA`／band 0、harbor region 0・water 0・depth `NO_DATA`、breakwater region 0、cape region 0・exposure 0・index 0）を書き、**新sentinel・新field id・新field semanticなし、`FIELD_IDS`の集合と順序も不変**（sidecar shapeをintentに依存させない）。サイズ0ではblueprintがtransition planを封印しないためcompositorを持たず、`composeAt`が既存`CompositionSample.outside()`を返す。**D5**により`ProductionRoutePreconditionsV2`の4 pipeline全てのrequired companion集合が空になり、クラス・API・rule id `v2.design.route-companion-missing`・`design-support-lint-v1` contract・JSON shapeは**すべて不変**（contract bumpなし）。`V2-19-08`が報告していた`PLAIN`単体／beach単体の`NOT_SELECTABLE`＋companion不足は`SELECTABLE`へ変わる（制約が実際に無くなったことの反映）。conformance caseは`harbor-cove-64-honored-beach`（サイズ1。`V2-18-13`の規則`active ? composed : macro-background`でmask再生成、sha `9127fa3e…`、背景比42 cell変化。dev-only helper `RegenerateBeachOnlyCoastalMaskExampleTest`同梱）と`harbor-cove-64-honored-coastless`（サイズ0。`PLAIN`＋既存maskを**byte同一で再利用**し、公開`ACTUAL_LAND_WATER`が宣言maskと4096/4096完全一致、HARD EDGEはmacro foundation単独で充足）を常設追加し、negativeはlegacy baseline＋部分集合のowner gate拒否・同一kind重複・mask矛盾（既存rule、追加なし）を固定した。portfolioのmeasurementはcontributorごとに任意化し（`beach`／`backshorePlains`は`Optional`、`arms`は宣言なしで空）、各caseの`declaredCoastalKinds`と測定の有無を先に照合するため黙ったcontributor脱落は依然失敗する。`MacroFoundationPhaseGateV2Test`（V2-18-12、人間承認済みgate）は**4種宣言caseだけ**へfilterし（gated 6件をdrift guardでpin）、gateのassertionを弱めていない。**capability昇格なし**: `PLAIN.standalone_usage`はADR 0040 D7により`PARTIAL`据え置き（単体構成のmaterialization証拠が未取得）。catalog注記はsealed checksum不変のため据え置き、訂正は[current-limitations](current-limitations.md)と証跡文書へ置いた。**既存4種fixtureのtile semantic checksum `20318e6c…`は絶対pinで不変**、容器byte・registry checksum・`public-dispatch-reachability-v1` projection・sealed catalog・Release format／capability・v1 goldenも不変。新Schema・新capability・新artifact format無し。full `./gradlew test`／`./gradlew build` PASS。証跡は [V2-19-09 audit](audits/v2-19-09-coastal-contributor-subset.md) |
| `V2-19-10` | 01,07 | Material／paletteのblock反映。environment material planをcanonical block resolverへ接続し、hard-coded 11 stateをprofile-driven allowlistへ置換、environment validationの定数healthy samplerを実field読みへ是正 | **完了（2026-07-24）**。semantic checksum変更はADR承認範囲、任意block state・外部scriptは不許可、ZONE_LABEL→material zoneは非Scope — いずれも充足。`SurfaceMaterialProfileV2`（`core.v2.material`、`surface-material-profile-v1`）がsurface columnのblock語彙を**閉じた13 role catalog／11 distinct state**（column tier `STRUCTURAL`／`NATURAL_SURFACE`／`NATURAL_SUBSURFACE`／`BUILT_STRUCTURE`付き）として宣言し、resolverの11 literalを置換した。全roleちょうど1回のmapを要求し、各stateは`EnvironmentBlockStateCatalogV2.requireKnown`を通るため**任意block state・非canonical識別子・NBT payloadは構築時に拒否**され、外部script／式は評価しない。`CoastalSurfaceFieldsV2`／`FoundationSurfaceFieldsV2`は「cell→role」だけを決め、block決定は`SurfaceMaterializationV2`（`surface-material-binding-v1`）が持つ。blockは凍結済みV2-4-07 rule tableの写しで2段に決まる: (1) roleのbase assignment（beach＝sand、cape＝rockという形状由来の意味はここにしかない）、(2) environment material bind時、解決済みsemantic material classが**wet／snow variant**のcellだけ`MinecraftPalettePlanV2`の`SURFACE` mappingで置換（exposed variantはoverrideを宣言しない）。対象は`NATURAL_SURFACE` roleのみで、structural・built structure（breakwater crest／core）・subsurfaceはprofileの状態を保つため、**environment materialはsolid質量もfluidも動かせず変化はblock effect class `MATERIAL`だけ**である。base assignmentごとpaletteへ委ねない理由は、共有pipelineのuniform geology priorがROCKを返す限りbeachがstoneになるためで、rule table自身がbase assignmentとconditional overrideを別rule idで分けている。`EnvironmentSurfaceMaterialV2`（`environment-surface-material-v1`）はrelease-local domain全体のoverrideを**tile書き出し前に一度だけ**凍結し（1 cell 1 byte、distinct state最大4）、palette planを`requireMaterialProfilePlan`で束縛、sealed catalog外のclass code（geology no-data sentinel 65535を含む）を拒否する。定数samplerの廃止は`CoastalEnvironmentFieldStackV2`（`coastal-environment-field-stack-v1`）で、temperature／moisture（`ClimateFieldSamplerV2`）、wetness／salinity／hydroperiod（`WaterConditionFieldSamplerV2`）、snow cover（`SnowFieldSamplerV2`）、material class（`MaterialProfileResolverV2`）、lithology（`StrataExposureResolverV2`）、habitat（`EcologyPlacementResolverV2`）、elevation／land-water／slope（生成済み`CoastalSurfaceFieldsV2`）、marine distance（境界connected water componentからのbounded多始点BFS）、flow accumulation（同一runのfrozen routing result）を公開対象Blueprintのsealed planから測る。宣言fieldを持たない2入力は**明示のbounded derivation**として契約化した（topographic exposure＝water level上の高さ／40 block飽和、slope＝4近傍最大段差／8 block飽和。個別の風・日射modelは実装せず捏造しない）。宣言planが無いfeature scoped入力（river／lake／tidal proximity、tidal range、freshwater discharge、wetland／reef／island／canyon mask、feature material class、wall height）は「該当featureの宣言なし」の値のままで海岸から推測せず、これを正直に保つため`requireSharedEnvironmentOnly`は`meanderingRiverPlans`も拒否対象へ加えた（RIVER系はこのpipelineの`executableKinds`に無くdispatch到達不能、defence in depth）。`HydrologyPlanExportPipelineV2`はV2-19-05と同型に`prepareHydrology`／`completeHydrology`へ分割し、**final canonical block streamの生成点はtile書き出しの1箇所のまま**である。実測は`harbor-cove-64-honored`のbaseline `surface-2_5d` Releaseとの公開Release間cell diffで、`comparedCells=167936`／`changedCells=3191`（`MATERIAL` 3191、`SOLID_SHAPE` 0、`FLUID` 0）、露出surfaceはbaseline `{gravel 2251, grass 1062, stone_bricks 334, cobblestone 228, sand 221}`→environment `{cobblestone 3419, grass 343, stone_bricks 334}`（**定数samplerなら1状態しか残らない**ため空間変化のblock側証拠。海岸・海底はwet、内陸dryはbase assignment、built crestは不変）、locale `tr-TR`／timezone `Pacific/Chatham`再exportとのdiffは0 cell。計測は`FeatureMaterializationV2`（V2-19-01のgate計測コード）を用い、plan-only metricを代替evidenceにしない。公開配線leafでもFeature昇格でもないためportfolio caseは追加していない。**`surface-2_5d`／`hydrology-plan`／ADR 0037 adapterのterrain field／tile／block semantic checksumは不変**（絶対pin `20318e6c…`は通過）、変化するのは`environment-fields`のtile／block semantic checksumとenvironment validation／previewの容器byteのみで、ADR 0038 D9の範囲内のためamendment不要。新Schema・新capability・新artifact format・dispatch route追加・capability昇格なし、v1 goldenは不変。残存制約（uniform geology priorのためmudは出ずcobblestoneになる、植生はblock未到達、snow分岐はfixtureで未発火、`environment-fields`はCLI／Paper未到達）は証跡文書と [current-limitations](current-limitations.md) に明記した。証跡は [V2-19-10 audit](audits/v2-19-10-material-palette-block-reflection.md) |
| `V2-19-11` | — | Docs current-state同期。`implementation-roadmap.md`（次Task=V2-15-05表記）／`migration-plan.md`（次=V2-12-07表記）／`current-limitations.md`（Status行のV2-18-11表記）／READMEのreference image・image E2E表現をコードの接続状態へ同期し、実装状態matrixへ「公開到達性（CLI／Paper到達可否）」列を追加 | **完了（2026-07-24）**。`implementation-roadmap.md`（Status行／依存chain図／Phase Task構造表／末尾要約の`V2-15-05`次Task表記をV2-15-10完了・V2-18全13完了・V2-19-11完了・次Task`V2-19-12`へ更新し、chain図とPhase Task構造表へV2-18／V2-19の行を追加）、`migration-plan.md`（Status行の`V2-12-07`次Task表記を、V2-12はPhase gate完了済みでTrack Aの次は`V2-19-12`である旨へ更新）、`current-limitations.md`（Status行を`V2-19-11`完了・次`V2-19-12`表記へ更新。本文の技術記述は`V2-19-09`／`10`で既に同期済みのため他は不変）を是正した。READMEのBeta statusへV2-15-09／10（`hydrology-plan`／`environment-fields` shared overlay到達、RIVER offline production route）、V2-18（macro foundation必須化、foundation owner gate）、V2-19-03〜10（reference image設計修理、constraint source binding authoring、RIVER block materialization、`HEIGHT_GUIDE` consumer、`PLAIN` foundation producer、design support lint、[ADR 0040](../adr/0040-coastal-contributor-set-cardinality.md)によるcoastal 4種必須の緩和、material／palette block反映）の現状を追記した。「実装状態matrixへの公開到達性列追加」は`V2-19-01`／`05`／`07`が[current-feature-state-machine-registry.md](current-feature-state-machine-registry.md)の`public-dispatch-reachability-v1` projectionとして既に実装済みだったため、本Taskは新規列を実装せずこのprojectionへの参照をREADME／current-limitationsへ追加する形で対応した（二重実装回避）。production code・Schema・example・checksumの変更なし。full `./gradlew test`／`./gradlew build` PASS |
| `V2-19-12` | 07 | Coherent detail kernel設計ADR＋実装。空間的に連続なbounded・deterministic・multi-scale detail modifier（seed namespace／frequency／amplitude／halo／support radius／tile seam／CPU・memory budget固定）。cell-hash独立ノイズの流用はしない | **完了（2026-07-24）**。**ADR＋人間承認必須**（承認前に実装しない）。erosionは別Task・同時導入禁止 — いずれも充足。[ADR 0041](../adr/0041-coherent-detail-kernel.md)を起草し、同日の人間承認で**D2「background base-level cellのみ」／D3「request level optional」を明示採択**したうえで実装した。`CoherentDetailKernelV2`（`generator.v2.detail`、`coherent-detail-fixed-v1`）は**整数演算のみ**のmulti-octave value noise（octave格子間隔`W>>k`＋integer smoothstep bilinear）で、octave振幅は`a_k=floor(A·2^(K-1-k)/(2^K-1))`＝**振幅Aはhard bound**（`Σa_k≤A`で`\|value\|≤A`）、`maximumAdjacentStepMillionths=Σ(ceil(3a_k/W_k)+64)`を契約として公開する（同一振幅のcell-hash場は必ずこの上限を破る＝流用でないことが機械判定可能）。seed namespaceは`NamedSeedDeriverV2`でmedium別に固定し著者は選べず、closed formのため**halo／support radius=0でwhole==tiled**。適用点は`MacroFoundationV2.elevationMillionthsAt`の背景base-level分岐**だけ**で（ADR 0038 D2-2の優先順位表に行を増やさない）、`HEIGHT_GUIDE`指定cell・producer所有cell・modifier所有cell・land-water mediumには一切触れない。宣言面はrequest level任意`foundationDetail`（medium別振幅0..32／wavelength 2冪8..1024／octaves 1..6）で、`GenerationRequestV2`が宣言時にfail-closed（両振幅0・非2冪・grid<4・base levels不在・water level跨ぎ・extent逸脱を拒否、clampなし。override flagなし）、`MacroFoundationV2`が生成時per-cellで`v2.foundation.detail-out-of-contract`をdefence in depth判定する。CLI／Paperへ`request foundation-detail`（`request.edit`権限）を追加。常設conformance case `harbor-cove-64-honored-detail`（mask／intent byte同一、`foundationDetail`のみ追加）を追加し、公開Release間block diff `changedCells=4431`（`SOLID_SHAPE` 728／`FLUID` 1191／`MATERIAL` 2512、宣言effect class一致）、背景land／water surface Y各4値（非平坦）、4近傍段差≤1 block（coherent）、modifier列変化0・datum跨ぎ0、locale／timezone再exportのblock diff 0を実測した。`CoherentDetailKernelV2Test`が振幅hard bound・coherence上限・cell-hash対照（>4×bound）・determinismを固定。**`foundationDetail`絶対なしのrequestはterrain field／tile／block semantic checksum・容器byte・既存Release・sealed catalog・dispatch registry・reachability projectionいずれも不変**（absentはJSONへ書かない、tile pin `20318e6c…`通過）。新capability・新artifact format・dispatch route追加・capability昇格なし、v1 goldenは不変。full `./gradlew test`／`./gradlew build` PASS。**非Scope:** erosion／近傍filter（`V2-19-13`以降の別ID＋別ADR）、producer cell-hash micro reliefのkernel置換、modifier境界のtaper band、LARGE detail |
| `V2-19-13` | — | Blueprint拡張面積縮小のADR検討。`WorldBlueprintV2`のper-kind typed plan list（17 list＋singleton群）のclosed typed plan envelope／registryへの段階集約案と、4 pipelineのexecutableKinds単一ソース化 | **完了（2026-07-24）**。ADR `Proposed`著述までが本Task、実装は承認後の新ID、Schema ID・artifact type・checksumのin-place変更禁止 — いずれも充足。[ADR 0042](../adr/0042-blueprint-typed-plan-envelope.md)を`Proposed`で起草した（同日rev.2で実測に基づき事実訂正）。事実確定: (1) `WorldBlueprintV2`は17 per-kind typed plan list（`coastalFeaturePlans`〜`volcanicPlans`）＋7 singleton（`geologyPlan`〜`hydrologyReconciliationPlan`）を持ち、1 kind追加の編集点のうち**正規化代入・`uniqueMap` id重複検査・per-kind presence-consistency loop・per-plan／cross-plan hook配線・schema型定義は忘れてもsilentに通る**（`withCanonicalChecksum`・compiler呼出し側はcompile errorで自己防護。codecはJackson databindがrecord componentへ直接bindするため拡張面積に含まれない — recordとschema 25 plan keyの2正本lockstepが実態）、(2) cross-plan参照はcanyon→river・breakwater→basin・transition→contributors・waterfall→river・delta→trunkRiver・tidal⇔mangrove・coralReef→lagoon／pass・fjord→GLACIAL_MOUNTAIN_RANGE・volcanic→caldera／lava・hydrology variables／constraints→featurePlansの**11系統**、(3) **schema型付けは既に劣化しており直近3種`fjordPlans`／`mountainPlans`／`volcanicPlans`は`items: true`の無型**（実害は仮定でなく実測）、(4) 4 pipelineの`executableKinds`は基底coastal 4種＋pipeline別offline加算＋共有contract-only `BACKSHORE_PLAINS`という実態を4 descriptor＋dispatch registry定数の5箇所へ手書きコピーしている。提案は**D1**（新package `model.v2.plan`のclosed typed plan registry `BlueprintPlanTypeV2<P>`＋named static methodのcross-plan hook。目的は編集行数削減ではなく**silent欠落の0件化** — Java recordのcompact constructorは正規化代入をループ化できないため編集点は残るが、registry再検査の構築時fail-closed・reflection drift guard・schema型付けguardで全ての忘れをloudにし、`items: true` 3件も型付け追補で是正する。**record component保持でwire形・`canonicalChecksum`・component宣言順不変**）と**D3**（`executableKinds`単一正本化＝基底集合＋pipeline別加算表＋共有contract-onlyの3宣言として保持。集合・registry checksum・reachability projection不変）を各1 leafで実装する段階計画、**D2**（wire `plans`オブジェクト化）はschema `$id`・checksumを全損する一方Java側面積をほぼ縮めないため**本ADRでは不採用とし将来独立ADR＋schema version bump＋人間承認へ分離**。凍結はwire形・`canonicalChecksum`・schema `$id`・component宣言順・validation判定の意味（fail-closed方向の強化のみ許可）・dispatch route集合・route class。D5に実装leafのAcceptance（wire完全不変・validation等価＋強化のみ・完全性guard実証・dispatch不変・決定性）を規定した。**ADRはStatus `Proposed`で採択待ち**（実装は採択後の新ID、本Taskでは実装しない）。docs-only、production code・Schema・example・checksum・v1 goldenは不変。full `./gradlew test`／`./gradlew build` PASS |
| `V2-19-14` | 06 | mask⇔feature SOFT reconcile pre-pass ADR＋実装。feature geometryをmask shorelineへ決定論的にsnapする補正（補正量tolerance超は従来どおり拒否）で、mask⇔seed束縛によるauthoring負担を軽減 | **完了（2026-07-24、人間承認済み）**。**ADR＋人間承認必須**（HARD不発明原則との整合を明文化、承認前に実装しない）— 充足。[ADR 0043](../adr/0043-mask-feature-reconcile-pre-pass.md)を起草し、同日の人間承認で**D1「宣言feature集合全体への単一剛体平行移動」／D3「request levelのoptional `maskFeatureReconcile`（tolerance 1..8、opt-in）」を明示採択**したうえで実装した。根拠は[横断監査](../audits/cross-cutting-audit-2026-07-23.md) §4.2-4のmask⇔seed束縛（提案T-A2）で、`V2-18-13`／`V2-19-09`がいずれもmaskを`active ? composed : macro-background`で**再生成**した実績が示すとおり、唯一の実務手順が「geometryかseedを触ったらmaskを作り直す」だった。新設`MaskFeatureReconcileStageV2`（`core.v2.export`、contract `mask-feature-reconcile-v1`）が`CoastalSurfaceExportPipelineV2.prepare`のBlueprint compile直前で、1 runにつき**ちょうど1つ**の整数block平行移動`(dx,dz)`を選び`features[].geometry`の全control pointへ適用する。目的関数はHARD source無しで合成したcoastal activeセル`A`上の`Σ[composed.landWater ≠ mask(u+d)]`（`v2.coastal-transition-hard-conflict`が評価する集合と同一）で、mask no-dataは比較対象外（**maskが言っていないことを推測しない**）、active cellがdomain外／control pointが`[0,10⁶]`外になる候補は無効、全順序は`(disagreement, \|dx\|+\|dz\|, dz, dx)`の辞書式最小のため**既にmaskと一致しているgeometryは絶対に動かない**。正規化deltaは`Δn(d,extent)=sign(d)×floor((\|d\|×2×10⁶+(extent−1))/(2×(extent−1)))`で`Δn(+d)+Δn(−d)=0`が厳密に成立する。**HARD不発明原則との整合**: maskの値・digest・binding・no-data cellは1 bitも変更せず（一方向）、動かすのはSOFT側＝著者の宣言geometryのみで、`v2.coastal-transition-hard-conflict`／kernel不変条件／`surface-foundation-owner-gate-v1`／preflight／EDGE evaluatorの判定と閾値は不変である。**新しい拒否ruleを追加していない**（D4）: 補正量は候補集合の構成上つねにtolerance内で、tolerance内に一致する配置が無いrunは従来どおり既存HARD gateが同じ理由で拒否する。per-feature offset・回転・拡大縮小・形状変形を行わないため宣言済みrelation（`ENCLOSES`／`ADJACENT_TO`／`OVERLAPS`）の相対geometryは厳密に保存される。宣言面はrequest任意`maskFeatureReconcile { toleranceBlocks: 1..8 }`（Schema追加、absentでは書き出さない）で、`foundationBaseLevels`必須・tolerance 0拒否・`width×length×(2T+1)² ≤ 128,000,000`を宣言時にfail-closed検査（clampなし）、CLI／Paperへ`request mask-reconcile`（`request.edit`）を追加した。公開Releaseの`source/terrain-intent.json`には**整列後のgeometry**を封印し（Releaseが実際に含む地形を記述するため）、適用offsetは`Release2ExportResultV2.maskFeatureReconcile`→CLI／Paper summaryのreport-only表示に留める（Blueprint／manifestへは書かない）。あわせて`MacroFoundationStageV2.resolve`を`bindInputs`（map decode 1回）＋`resolve(inputs, request, intent)`（純関数）へ分割し、pre-passがmask解決後・producer構築前に走るようにした（挙動はabsent時完全不変）。常設conformance case `harbor-cove-64-honored-drift`（honored intentの全geometryへ`Δn(0,+2)`を適用、mask・request本体はbyte同一で再利用）を追加し、pre-passが`(0,−2)`を選んで**publish済みgeometryがhonoredと完全一致**、tile semantic checksumが絶対pin `20318e6c…`、`FeatureMaterializationV2`のbaseline比block diffが`changedCells=0`（`comparedCells=167936`）であることを実測した（`V2-19-01`が禁じたidentity slice流用ではない: Feature昇格を主張せず、比較対象は別intentから独立にexportした2つの公開Releaseで、差分ゼロ自体が測定対象）。実測report `[tolerance=4, offset=(0,−2), evaluatedCells=958, disagreementBefore=151, disagreementAfter=0, candidateOffsets=81, rejectedOffsets=65, translatedFeatures=5]`。negativeはpre-pass無し／tolerance 1でのexportが**既存rule**`v2.coastal-transition-hard-conflict`で拒否されること、宣言時の4条件（base levels不在・tolerance 0／9・予算超過）で、determinismは`tr-TR`／`Pacific/Chatham`再exportで固定した。`MacroFoundationPhaseGateV2Test`のgated caseは7→8へ拡大（gateを狭めていない）。**非Scope:** 形状（shape）reconcile＝手描きmaskへのcell単位追従、per-feature offset、回転・拡大縮小、sub-block探索、mask側の補正、`HEIGHT_GUIDE`のregistration、taper band、erosion、LARGE。目的関数がper-cell HARD述語の**推定器**である（重なりcellで乖離し得る）ことも隠さず記録した。新Schema `$id`・新capability・新artifact format・dispatch route追加・capability昇格なし、既存fixtureのterrain semantic checksum・容器byte・sealed catalog・v1 goldenは不変。full `./gradlew test`／`./gradlew build` PASS。証跡は [V2-19-14 audit](audits/v2-19-14-mask-feature-reconcile.md) |
| `V2-19-15` | — | **完了（2026-07-24）**。commit hygieneの改善根拠は[2026-07-23横断監査](../audits/cross-cutting-audit-2026-07-23.md) §6-6「commit hygiene」（提案T-Q2）で、実際のcommit履歴に情報量ゼロの先頭行（`test`／`tset`）が存在することを確認した。`AGENTS.md`へ新設「16. Commit message規約」を追加し、(1) Task Scopeの変更は`<Task ID>: <変更内容の要約>`（例: `V2-19-15: AGENTS.mdへcommit message規約を追加`）を先頭行とし複数Task IDを1 commitへ混在させない、(2) 単一Taskに紐付かない変更（横断監査文書、roadmap構造是正、CI／依存更新等）は`docs:`／`chore:`／`build:`／`audit:`のいずれかを先頭に置く、(3) `test`／`tset`／`修正`／`更新`／`作業`のように対象と内容を特定できない語だけの先頭行を禁止する、(4) 本規約は今後のcommitへ適用し既存commitのrebase／amend／force-pushによる書き換えは行わない、を規定した。[task-execution-guide.md](task-execution-guide.md) §5実行規則へ item 8としてAGENTS.md §16参照を追加した。production code・Schema・example・checksumの変更なし、docs-onlyのためfull `./gradlew test`／`./gradlew build` PASS。 | docs-only。既存commitの書き換えはしない。**Acceptance:** AGENTS.mdへcommit message規約（Task ID必須、情報量ゼロ先頭行禁止）を明文化した |
| `V2-19-16` | 01..15 | Phase gate。全leaf再検証、materialization gate発効とV2-15再開状態の確認、full clean suite | **完了（2026-07-24、人間承認済み）**。親Phaseを閉じる唯一のTask。新gate test `InputIntegrityPhaseGateV2Test`（`integration.v2.conformance`、6 method／15実行）が全10 portfolio caseを自らproduction export serviceでexportし、atomic publish済みRelease成果物だけを再測定してphase固有主張を固定する — (1) **portfolio全caseのPhase gate化**（`V2-18-12`が4-contributor 8 caseへ限定していた資格を、ADR 0040 subset case `harbor-cove-64-honored-beach`／`-coastless`を含む全caseへ拡大。V2-18-12 gateコード明記の引き継ぎの実行で既存assertionは弱めない。宣言contributor集合＝sealed intent実宣言集合、測定の有無＝宣言の有無、サイズ0でもHARD EDGE非空・充足、登録済み非適合ゼロ、ROCKY_CAPE宣言caseはsea stack会計）、(2) **materialization gate発効**（`public-dispatch-reachability-v1`でPRODUCTION_CONNECTED＝coastal 4／OFFLINE_PRODUCTION＝`{RIVER, MEANDERING_RIVER, PLAIN}`＝全MATERIALIZED・**PLAN_ONLY空集合**・CONTRACT_ONLY＝`{BACKSHORE_PLAINS}`の完全一致、routed 3 kindのblock effectをbaseline比で再実測し宣言effect class完全一致、plan column PASSでもidentity streamは必拒否）、(3) fail-closed spine契約pin（V2-18スパイン不変＋`production-dispatch-registry-v2`／`design-support-lint-v1`／全pipelineのcompanion要求空（ADR 0040 D5）／`surface-material-profile-v1`系3件／`coherent-detail-fixed-v1`／`mask-feature-reconcile-v1`）、(4) **V2-15再開状態**（design lint surface＝dispatch registry投影のchecksum一致、`LAKE`／`OXBOW_LAKE`は`NOT_PUBLICLY_DISPATCHABLE`のまま先取りなし、composition NORMATIVE 7／PROVISIONAL 53で`LAKE`はPROVISIONAL＝per-leaf義務の根拠。期待値更新はregistry／ADR正本と同一commitのみ）、(5) capability・寸法の無昇格（paper SUPPORTED exact set＝coastal 4、`RIVER`／`MEANDERING_RIVER`のPaper 5列EXPERIMENTAL、`PLAIN`のPaper 5列UNSUPPORTED＋standalone PARTIAL、dimension `measured()`、sealed catalog checksum一致、Release 2 capability list／prefix完全一致）。全leaf再検証はfull clean suite `./gradlew clean test build`（239 test class／1461 tests／0 failures／0 errors／12 skipped全分類済み）PASS、`scripts/ci/v2-19-02-inventory-input-check.sh` 8/8 PASSED再現。gateは計測のみでtracked fileへ書かず、terrain semantic checksum・容器byte・sealed catalog・dispatch registry・reachability projection・v1 goldenは全て不変。証跡は [V2-19-16 Phase gate audit](audits/v2-19-16-phase-gate.md)。**(a) portfolio gate範囲拡大 (b) `V2-19-16`完了 (c) V2-19親Phase閉鎖は2026-07-24の人間承認（不可分のatomic decision）で有効化され、V2-19親Phaseは閉じた** |
