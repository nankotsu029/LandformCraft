...
> Status: V2-0〜V2-13の既登録Taskは、V2-8のHOLDと親Phase人間承認待ちを除き完了し、V2-14は`V2-14-01`〜`03`まで完了している。2026-07-22の[全地形カタログ監査](../audits/terrain-catalog-full-audit-2026-07-22.md)を受け、Track E続編`V2-15`／`V2-16`、Track A placement evidence follow-up `V2-17`を登録した。`V2-15-01`〜`V2-15-08`を完了し、ADR 0036に従うcanonical target registry、strict Schema／codec、明示migration、registry-driven production dispatch spine、`hydrology-plan`／`environment-fields`／`sparse-volume` shared production pipelineを固定した。2026-07-22の[macro foundation／intent conformance監査](../audits/macro-foundation-conformance-audit-2026-07-22.md)を受け、Track Aへ`V2-18`（12 Task、§18）を登録し、V2-15のproduction export／placement昇格系leafへ`V2-18-09`完了＋composition role登録を前提とするstage gateを適用した。**現在の次Taskは Track A `V2-18-01`（diagnostic gate契約正常化）／Track E `V2-15-09`（V2-9／10 foundation surface export adapter）である。** この文書はTask ScopeとAcceptance gateの正本、進捗状態の正本は [docs/roadmap.md](../roadmap.md)、モデル割当は [model-assignment.md](model-assignment.md) である。
...
| A | V2-18 Macro foundation／intent conformance | 12 | `V2-18-01` | `V2-18-12` |

合計230 Taskである（従来144 Task＋`V2-14-03`＋V2-15の47 Task＋V2-16の19 Task＋V2-17の7 Task＋V2-18の12 Task）。Task追加時は既存IDを振り直さず、該当Phaseへ新しい未使用IDを追加する。Track間は依存を明示したTaskだけが待ち、それ以外は並行実行できる。ただし同一ファイルを複数エージェントが同時編集してはならず、`format.v2.release`等の共有領域を変更するTaskは直列にする。
...
| `V2-15-09` | 05 | V2-9／10 foundation surface export adapter | 既存capabilityへ写像。新capability／formatが必要ならADRで停止 |
| `V2-15-10` | 03,06 | `RIVER`＋legacy meandering subtype public wiring | `BRAIDED`はV2-16。既存meander checksum不変 |
| `V2-15-11` | 03,06 | `LAKE`＋oxbow cutoff subtype public wiring | dam reservoirはV2-16 |
...
| `V2-15-47` | 10..46 | Phase gate、既存20＋全leaf E2E、full clean baseline | 親Phaseを閉じる唯一のTask。欠陥は隠さず新IDへ分離 |
...
- **共通test／D/M/S:** V2-15共通gateに加え、childがparent boundsを越えないこと、overlap／不正交差のfail closed、配置順とseed派生の順序不変、preview legend／manifest strict read-back、1000角bounded working setを検証する。通常Taskは対象test＋`test`／`build`、full clean portfolioは`V2-16-19`だけが実行する。
...
| `V2-16-19` | 02..18 | Phase gate、full clean baseline、canonical count再確認 | 親Phaseを閉じる唯一のTask |
...
## 18. V2-18 Macro foundation and intent conformance（Track A、12 Task）
...
| `V2-18-08` | 02 | Macro foundation契約ADR。全surface XZ cellにeffective foundation ownerを要求するproduction契約、`MacroLandWaterTopologyPlanV2`（V2-9-12）／`SurfaceFoundationPlanV2`＋merge kernel（ownerless拒否）の昇格差分、`FeaturePrimaryRoleV2`（catalog意味）と分離したcomposition role軸（foundation producer／surface modifier／carver／overlay等）の要否と60 kind対応表、資源budget、EDGE等からの内部境界推測を禁止するresolution policy | `V2-18-04`の結果はADRレビュー入力でありblocking依存ではない。新capability／formatが必要なら停止 |
...
| `V2-18-11` | 04 | Intent-conformance E2E portfolio。`coastal-fishing-map`の形状assertion（EDGE実測、beach↔backshore land連続性、breakwater両armのland接続）を常設回帰化し、以降のleaf完了ごとに複合fixture（山＋川＋湖、島＋reef等）を蓄積 | portfolioのPhase gate昇格は`V2-18-12`が行う |
| `V2-18-12` | 01..11 | Phase gate。全leaf再検証、conformance portfolioのgate化、`V2-15-47`／`V2-16-19` Acceptanceへのintent-conformance追記（人間承認）、V2-15 stage gate解除判断 | 親Phaseを閉じる唯一のTask。欠陥は隠さず新IDへ分離 |

## 19. 親Phaseを閉じる規則

`V2-2-12`、`V2-3-15`、`V2-4-15`、`V2-5-18`、`V2-6-19`、`V2-7-07`、`V2-8-08`、`V2-9-14`、`V2-10-09`、`V2-12-07`、`V2-15-47`、`V2-16-19`、`V2-17-07`、`V2-18-12`だけが親Phaseを完了へ変更できる。V2-13／V2-14は統合監査Task未設定であり、全Task完了後も親Phaseの完了宣言には人間承認を要する（V2-13-05は条件不成立により2026-07-21に`NOT_APPLICABLE_BY_MEASUREMENT`で不実行クローズ済み）。先行Taskが完了しても、次のどれかが残れば親Phaseは `進行中` である。
