...
合計231 Taskである（従来144 Task＋`V2-14-03`＋V2-15の47 Task＋V2-16の19 Task＋V2-17の7 Task＋V2-18の13 Task）。Task追加時は既存IDを振り直さず、該当Phaseへ新しい未使用IDを追加する。Track間は依存を明示したTaskだけが待ち、それ以外は並行実行できる。ただし同一ファイルを複数エージェントが同時編集してはならず、`format.v2.release`等の共有領域を変更するTaskは直列にする。
...
| `V2-18-13` | 11 | `coastal-honored-400`の東breakwater arm landfall非適合の是正。宣言landfall（0.61, 0.47）がrocky cape西端へ届かずarm全体が孤立land componentになる状態を、fixture geometry是正と`V2-18-09` land-water mask（composed出力∪macro構図）の再生成で解消し、portfolioの期待を両arm接続へ更新する | `V2-18-11`のportfolioが確定した非適合の是正専用。generator挙動・compositor契約・Release formatは変更しない。maskを再生成できない場合は停止し、原因Taskへ差し戻す |
| `V2-18-12` | 01..11,13 | Phase gate。全leaf再検証、conformance portfolioのgate化、`V2-15-47`／`V2-16-19` Acceptanceへのintent-conformance追記（人間承認）、V2-15 stage gate解除判断 | 親Phaseを閉じる唯一のTask。欠陥は隠さず新IDへ分離 |
