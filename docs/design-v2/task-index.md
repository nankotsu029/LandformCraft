...
| C | V2-13 MEDIUM 1024 enablement／measurement | 5 | `V2-13-02`（`V2-13-01` 2026-07-21完了） | `V2-13-05`（実測により条件不成立で保留） |
| B | V2-14 Image fidelity wiring follow-up | 2 | —（2 Task完了、`V2-14-02` 2026-07-21完了） | —（Phase gate Task未設定、完了宣言は人間承認要） |
| D | V2-9 Terrain foundation | 14 | `V2-9-01` | `V2-9-14` |
| E | V2-10 Deferred terrain families | 11 | `V2-10-01` | `V2-10-09` |

合計144 Taskである（`V2-12-07` gate検出のfollow-up defect `V2-12-11`を追加）。Task追加時は既存IDを振り直さず、該当Phaseへ新しい未使用IDを追加する。Track間は依存を明示したTaskだけが待ち、それ以外は並行実行できる。ただし同一ファイルを複数エージェントが同時編集してはならず、`format.v2.release`等の共有領域を変更するTaskは直列にする。
...
- **非Scope:** SLO宣言、supported化、Paper配置、LARGE。
- **Gate／停止:** 実測evidenceのコミット。budget超過が判明した場合は超過箇所を報告して停止する。

### V2-13-04 1024×1024 FAWE placement measurement（旧PROPOSAL-B5）

- **状態:** 未着手。
- **目的／前提:** V2-11-04／05と同一手法（FAWE 2.15.2単独、専用高memoryホスト、単独Paper JVM、RCON、2 pass＋Recovery drill）で1024×1024の実world配置を実測する。前提は`V2-13-01`（工程別計測）、`V2-13-03`。
...
- **状態:** 保留（条件不成立、2026-07-21）。`V2-13-01`実測で最大ボトルネックは`PLACEMENT_STAGE_DURATION_APPLY`（~79.5%、FAWE apply）と判明し、per-block `Map` を持つ SNAPSHOT＋UNDO は合計 ~19% にとどまった。条件付き承認の前提（Map／allocationが主要ボトルネック）を満たさないため現Scopeでは実行しない。最適化対象はFAWE apply経路であり、着手には別途Task化と承認が必要。証跡: [V2-13-01 evidence](audits/v2-13-01-stage-instrumentation-evidence.md)。
...
- **Gate／停止:** 全checksum不変＋改善の実測。ボトルネックが別工程だった場合は実行せずクローズし、代替Taskを提案する。
...
`V2-2-12`、`V2-3-15`、`V2-4-15`、`V2-5-18`、`V2-6-19`、`V2-7-07`、`V2-8-08`、`V2-9-14`、`V2-10-09`、`V2-12-07`だけが親Phaseを完了へ変更できる。V2-13／V2-14は統合監査Task未設定であり、全Task完了後も親Phaseの完了宣言には人間承認を要する（V2-13-05は条件不成立時に不実行クローズを許す）。先行Taskが完了しても、次のどれかが残れば親Phaseは `進行中` である。
