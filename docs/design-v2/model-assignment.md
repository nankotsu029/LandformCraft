...
現在実行可能なPhaseはV2-15（Track E。公開配線leafの`V2-19-01` stage gateは2026-07-23完了・人間承認で解除済み、以後はmaterialization義務込みで実行）である。V2-19は2026-07-24の人間承認でPhase gate `V2-19-16`が完了し、全16 Task完了で閉鎖した。V2-18は2026-07-23の人間承認でPhase gate `V2-18-12`が完了し、全13 Task完了で閉鎖した。V2-19は2026-07-23の[横断監査](../audits/cross-cutting-audit-2026-07-23.md)人間承認で登録し、全16 Taskを2026-07-24までに完了した（§9.3）。V2-16／V2-17は依存待ち、V2-8-03〜08はHOLDで§10に置く。完了済みTask（V2-0〜V2-7、V2-9〜V2-14、V2-15-01〜10、V2-18全13、V2-19全16）は本表へ掲載しない（履歴は§15とroadmap）。
...
### 9.2 V2-15 Canonical catalog／existing-generator wiring（Track E、残り36）

進捗: `V2-15-01`〜`13`完了（13/50。V2-15-11で新IDの`V2-15-48`、V2-15-13で新IDの`V2-15-49`／`V2-15-50`を追加したため総数が47→50になった）。次のTaskは`V2-15-14`。**[ADR 0039](../adr/0039-offline-production-route-eligibility.md) Gate 0 は2026-07-23の人間承認で`Accepted`（候補A採択: offline production route は dispatch `production-dispatch-registry-v1→v2` contract bump で追加）となり、`V2-15-10`が候補Aパターンの最初の適用leafとして完了した**。`PRODUCTION_CONNECTED`（Paper込み完全接続＝coastal 4）の意味・Paper `SUPPORTED` exact setは不変（`RIVER`／`MEANDERING_RIVER`／`LAKE`／`CANYON`はexport-SUPPORTEDのまま`OFFLINE_PRODUCTION`route、Paper `paper_apply`はEXPERIMENTALで昇格なし）。`V2-15-11`（2026-07-24完了）は`LAKE`基本（CLOSED basin）を同パターンで配線し実体化した。oxbow cutoff subtype（`OXBOW_LAKE`）はLAKEと同一kernelを共有しない構造的に別の生成経路（関係graph束縛）のため、task-execution-guide.md §4の「1 Taskで2個目の独立generatorを抱えない」規約により新ID`V2-15-48`へ分離した。`V2-15-12`（2026-07-24完了）は`CANYON`基本（既存`MEANDERING_RIVER`reachへHARD WITHINで束縛される横断面、CARVE_SOLIDのみでfluid非所有）を同パターンで配線し実体化した。着手中、`HardPreflightGateV2`のHARD relation「production-connected」判定が`DiagnosticGateContractV2.isProductionConnected`（coastal 4限定のPaper完全接続axis）だけを見ており、ADR 0039の`OFFLINE_PRODUCTION`route宛のHARD relationを誤って`v2.preflight.hard-relation-unconsumed`拒否する潜在的gapを発見し、`HardPreflightGateV2`が`PublicDispatchReachabilityV2`も参照するよう是正した（`isProductionConnected`自体の意味・戻り値は不変）。`V2-15-13`（2026-07-24完了）は`WATERFALL`のsurface側（host `MEANDERING_RIVER`へHARD `ON_PATH_OF`で束縛される落差のうち、2.5D surfaceが所有するplunge basin）を同パターンで配線し実体化した。同leafが対象としていた`WATERFALL_VOLUME` overlayは、`sparse-volume` capability prefixが含む`environment-fields` pipelineが`V2-19-10`の明示判断でhost river同梱Blueprintをfail-closed拒否するため完了できず、environment field stackの実測化（`V2-15-49`）とvolume overlay配線（`V2-15-50`）へ分離した。残りの`V2-15-14`〜`46`、`48`〜`50`は同一パターンをコピーする。

**Stage gate（全行共通、2026-07-23解除済み）:** 一律保留（`V2-18-09`完了までの待機）は`V2-18-12`の裁定と2026-07-23の人間承認で解除された。以後、production export／placement昇格系の作業・Acceptanceは各配線leafの**per-leaf義務**として次を負う: (1) 対象kindのcomposition profileをfield監査でPROVISIONAL→確定する（ADR 0038 D4。`CompositionProfileRegistryV2`にNORMATIVE登録済みなのは現状coastal 4種＋`PLAIN`／`HILL_RANGE`のみ。対応表の変更が必要ならADR 0038 amendmentを先行させ、Phase gate期待値はregistry変更と同一commitで更新する — test期待値だけの変更は禁止）、(2) 新規接続kindのintent-conformance portfolio caseを追加し、portfolio全case適合（登録済み非適合ゼロ）を維持する。registry／Schema／codec／offline plan／determinism系の作業は従来どおり制約なし。加えて ADR 0039 Accepted 後は、承認された offline production route パターンだけを 10〜46 でコピーする（A/B 混在禁止）。**さらに2026-07-23の[横断監査](../audits/cross-cutting-audit-2026-07-23.md)人間承認により、(3) 公開配線leaf（production wiring系）は`V2-19-01`が確立したsemantic materialization義務 — 対象Featureのblock effect class宣言と、final canonical block streamからbaseline Releaseとの非空効果・形状conformance測定（`FeatureMaterializationV2`によるportfolio case）、plan-only metricとblock metricの別欄化 — をAcceptanceへ含む。`V2-19-01`は2026-07-23に完了（人間承認済み）し、この着手待ちは解除された**（registry／Schema／codec／offline plan／determinism系leafは従来どおり制約なし。義務の正本はTask Index §15.1）。
...
| V2-15-11 | **完了（2026-07-24）** | `LAKE` wiring（materialization義務込み、oxbow subtypeは`V2-15-48`へ分離） | 中高（hydrology geometry＋block実体化） | Sonnet 5 | High | Grok 4.5 | — | §6既定 |
| V2-15-12 | **完了（2026-07-24）** | `CANYON` wiring（materialization義務込み、HardPreflightGateV2のOFFLINE_PRODUCTION relation gap是正を含む） | 中（通常public wiring） | Grok 4.5 | — | Sonnet 5 | — | §6既定 |
| V2-15-13 | **完了（2026-07-24）** | `WATERFALL` surface wiring（plunge basin materialization。volume overlayは`V2-15-50`へ分離） | 中高（host channelとのownership分離） | Opus 4.8 | High | — | — | §6既定 |
| V2-15-49 | 依存07 | environment field stackのhydrology proximity実測化（river／canyon一律拒否解除） | 中高（block state経路へ波及） | Sonnet 5 | High | Grok 4.5 | — | material class変化の範囲が争点ならOpus |
| V2-15-50 | 依存08,13,49 | `WATERFALL_VOLUME` sparse-volume overlay wiring | 中高（CSG ordering接続） | Sonnet 5 | High | Grok 4.5 | — | ordering不変条件が争点ならOpus |
...
| V2-15-47 | 依存10..46,48 | Phase gate（full suite、既存20＋全leaf E2E） | 最高（48 Task Phaseの最終監査／false promotion防止） | **Fable 5** | Highest available | Grok 4.5 | Opus 4.8＋**人間承認** | — |
| V2-15-48 | 依存11 | `OXBOW_LAKE` cutoff subtype wiring（materialization義務込み、関係graph束縛の独立生成経路） | 高（HARD `SUPPORTED_BY`／`ORIGINATES_AT` relation graph、既存`OxbowLakePlanCompilerV2`／`OxbowLakeGeneratorV2`の再利用、block実体化） | Opus 4.8 | High | Grok 4.5 | — | §6既定 |

割当根拠: 旧割当はwiring系の大半をOpus相当以上としていたが、`V2-15-05`〜`09`でdispatch spine・shared pipeline・export adapterが完了済みのため、確立パターンへ載せるだけのwiringはGrok（通常）／Sonnet（互換・checksum・subtype解釈あり）へ引き下げた。Opusは数学・memory・ordering・public Schema新設を含む行だけに残す。`V2-15-47`は公開カタログ全面の最終監査でfalse promotionが製品保証へ直結するためFableとする。`V2-15-48`は関係graph束縛（host＋parent river）を要する構造的に別の生成経路の初配線であり、通常wiring leafより設計判断の余地が大きいためOpusとする。

### 9.3 V2-19 Input integrity and block materialization（Track A主担当・横断、16 Task）

進捗: 全16 Task完了（16/16。Phase gate `V2-19-16`は2026-07-24人間承認で完了し親Phaseを閉じた）。`V2-19-01`は2026-07-23実装・検証、同日の人間承認でAcceptance遡及変更のatomic decisionを承認し、V2-15公開配線leafのstage gateは解除済みである（詳細は[Task Index §19](task-index.md)の`V2-19-01`行とroadmap Track A行）。`V2-19-02`（Gradle test input修正）は2026-07-23に完了した（走査rootを`test` taskのinputへ宣言、cache有効／無効の同値性を`scripts/ci/v2-19-02-inventory-input-check.sh`で実測固定、判定規則は不変。割当はGrok 4.5だがOpus 4.8で実行した）。`V2-19-03`（reference-image public design修理）は2026-07-23に完了した（宣言画像のsecure準備→provider handoff、実CLI／Paper E2E、任意`expectedSha256`。割当はSonnet 5だがOpus 4.8で実行した）。`V2-19-04`（generic constraint source＋Intent binding authoring）は2026-07-23に完了した（3 role共通のsource宣言verb＋CLI専用のbinding生成・確認verb、source→binding→compile→consumerの実CLI E2E。割当はSonnet 5だがOpus 4.8で実行した）。`V2-19-05`（RIVER block materialization）は2026-07-23に完了した（tile書出し前のglobal route freeze、`CARVE_SOLID`→`ADD_FLUID`責務分離、宣言interactionのfail-closed化、final tile streamからのbed depth／water continuity／source-mouth reachability／leak envelope実測、`RIVER`／`MEANDERING_RIVER`の`MATERIALIZED`表示。割当どおりOpus 4.8で実行した）。`V2-19-06`（HEIGHT_GUIDE macro foundation consumer）は2026-07-23に完了した（guide＞medium base levelの優先順位でbackground elevationを決定、no-dataはbase levelへfallback、constraint map要求をrole別へ緩和、height desired／actual／residualの入力guide束縛とconformance `RasterResidual`有効化、guide無しrequestのbyte不変を直前treeの実測checksumでpin。割当どおりOpus 4.8で実行した）。`V2-19-07`（foundation producer tier＋PLAIN vertical slice）は2026-07-24に完了した（`ProducerLayer`の初構築、footprint内replacement、4条件のfail-closed kernel不変条件、per-cell foundation owner index、`LandformPlainModuleV2`のcatalog登録とADR 0039 Candidate Aに従う`PLAIN`の`OFFLINE_PRODUCTION` route＋`MATERIALIZED`表示、公開Releaseからのblock実測、ADR 0037 adapterを公開dispatchへ載せない裁定。割当どおりOpus 4.8で実行した）。`V2-19-08`（design-time support lint）は2026-07-24に完了した（provider呼出し前のreachable集合提示＝dispatch registry投影・provider応答後のdispatch dry-run・`NON_GATING` finding 5 ruleのdesign audit記録とCLI／Paperサマリ表示、runtime precondition（coastal 4種必須）の公開表明、fail-closed化なし。割当はGrok 4.5だがOpus 4.8で実行した）。`V2-19-09`（coastal 4種必須の緩和）は2026-07-24に完了した（[ADR 0040](../adr/0040-coastal-contributor-set-cardinality.md)を起草し同日の人間承認でD1「サイズ0〜4」を明示採択、`CoastalGeneratorRuntimeV2.REQUIRED`削除、欠落kindはcanonical OUTSIDE値、`ProductionRoutePreconditionsV2`のcompanion要求が空へ、subset／coastless conformance case追加、既存4種fixtureのchecksum不変。割当どおりOpus 4.8で実行した）。`V2-19-10`（material／paletteのblock反映）は2026-07-24に完了した（閉じた13 role／11 state の`surface-material-profile-v1`がhard-coded literalを置換し allowlist を構築時強制、`SurfaceMaterializationV2`がbase assignment＋wet／snow conditional overrideの2段でblockを決め、`EnvironmentSurfaceMaterialV2`がtile書き出し前にoverrideを凍結、`CoastalEnvironmentFieldStackV2`が定数healthy samplerを実field読みへ置換、`prepareHydrology`／`completeHydrology`分割、公開Release間cell diffで`MATERIAL` 3191／solid 0／fluid 0を実測、`surface-2_5d`／`hydrology-plan` semantic checksumは不変。割当はSonnet 5だがOpus 4.8で実行した）。`V2-19-11`（docs current-state同期）は2026-07-24に完了した（`implementation-roadmap.md`／`migration-plan.md`／`current-limitations.md`のStatus行にあった陳腐化した「次Task」表記（`V2-15-05`／`V2-12-07`／`V2-18-11`）を現在の状態（V2-15は10/47、V2-18は13/13、V2-19は11/16）へ更新し、READMEのBeta statusへV2-15-09／10・V2-18・V2-19-03〜10の到達性・実体化状況を追記した。「実装状態matrixへの公開到達性列追加」は`V2-19-01`／`05`／`07`が[current-feature-state-machine-registry.md](current-feature-state-machine-registry.md)の`public-dispatch-reachability-v1` projectionとして既に実装済みのため、本Taskは新規列を実装せずこのprojectionへの参照を追加する形で対応した。production code・Schema・checksum不変。割当どおりHaiku相当のdocs-onlyタスクとして実行した）。`V2-19-12`（coherent detail kernel）は2026-07-24に完了した（[ADR 0041](../adr/0041-coherent-detail-kernel.md)を起草し同日の人間承認でD2「background base-level cellのみ」／D3「request level optional」を明示採択、`CoherentDetailKernelV2`＝整数演算multi-octave value noise・振幅hard bound・隣接段差上限の公開・cell-hash対照、適用点は`MacroFoundationV2`の背景base-level分岐のみ、request任意`foundationDetail`＋CLI／Paper `request foundation-detail`、常設case `harbor-cove-64-honored-detail`でblock diff `changedCells=4431`／coherent段差≤1／modifier不変／determinismを実測、`foundationDetail`絶対なしのbyte不変。割当どおりOpus 4.8で実行した）。`V2-19-13`（Blueprint拡張面積縮小のADR検討）は2026-07-24に完了した（[ADR 0042](../adr/0042-blueprint-typed-plan-envelope.md)を`Proposed`で起草。`WorldBlueprintV2`の17 typed plan list＋7 singletonの線形拡張面積と4 pipelineの`executableKinds`重複を確定し、D1内部registry集約（wire・`canonicalChecksum`不変）／D3 executableKinds単一ソース化を各1 leafで実装する段階計画を提案、wire形変更D2はschema `$id`・checksum影響のため将来独立ADRへ分離。実装は採択後の新IDで、本Taskは`Proposed`著述までがScope。docs-only、production code・Schema・checksum不変。割当どおりOpus 4.8で実行した）。`V2-19-14`（mask⇔feature SOFT reconcile pre-pass）は2026-07-24に完了した（[ADR 0043](../adr/0043-mask-feature-reconcile-pre-pass.md)を起草し同日の人間承認でD1「宣言feature集合全体への単一剛体平行移動」／D3「request level optional `maskFeatureReconcile`（tolerance 1..8、opt-in）」を明示採択、`MaskFeatureReconcileStageV2`がBlueprint compile直前に1つの整数block offsetを選び全feature geometryへ適用、maskは一方向で不変・既存fail-closed gateは判定も閾値も不変・pre-pass専用の拒否ruleはゼロ、既に一致しているgeometryは全順序の定義により不動、常設case `harbor-cove-64-honored-drift`でpublish済みgeometryのhonoredとの完全一致・tile pin `20318e6c…`・block diff 0を実測、negativeはpre-pass無し／tolerance 1が既存ruleで拒否。割当どおりOpus 4.8で実行した）。`V2-19-15`（commit message規約）は2026-07-24に完了した（`AGENTS.md`へ§16を新設しTask ID必須・情報量ゼロ先頭行禁止を規定、docs-only）。`V2-19-16`（Phase gate）は2026-07-24に実装・検証を完了した（新gate test `InputIntegrityPhaseGateV2Test`＝6 method／15実行が、portfolio全10 case（ADR 0040 subset含む）のPhase gate化、materialization gate発効（公開route 3 kind全MATERIALIZED・PLAN_ONLY空集合・identity stream必拒否）、V2-18スパイン＋V2-19追加契約のpin、V2-15再開状態、capability無昇格をgate自身のexportした公開Releaseの再測定で固定。full clean suite 239 class／1461 tests／0 failures／12 skipped全分類PASS、`v2-19-02-inventory-input-check.sh` 8/8 PASSED。割当どおりFable 5で実行した。証跡は[V2-19-16 Phase gate audit](audits/v2-19-16-phase-gate.md)）。**同日の人間承認（不可分のatomic decision）で`V2-19-16`は完了し、V2-19親Phaseは全16 Taskで閉じた**。Track Aの残りはV2-17（`V2-15-47`／`V2-16-19`完了待ち）のみである。Scope正本は[Task Index §19](task-index.md)。

| Task | 状態 | 内容 | リスク | 主担当 | Effort | 第一レビュー | 第二レビュー／人間gate | 昇格条件 |
|---|---|---|---|---|---|---|---|---|
| V2-19-05 | 完了（2026-07-23） | RIVER block materialization（bed carve＋water fill、responsibility分離、final tile検査） | 高（production spine＋checksum影響） | Opus 4.8 | High | Grok 4.5 | Sonnet 5 | 実行モデルOpus 4.8。river Releaseのtile／block semantic checksumのみ変化（意図どおり）、coastal既存Releaseは不変 |
| V2-19-06 | 完了（2026-07-23） | HEIGHT_GUIDE macro foundation consumer（D2-2範囲、1枚制約のrole別緩和、RasterResidual有効化） | 高（foundation契約＋決定性） | Opus 4.8 | High | Grok 4.5 | Sonnet 5 | — |
| V2-19-07 | 完了（2026-07-24） | Foundation producer tier＋PLAIN vertical slice（ProducerLayer初接続、candidate→effective、dispatch到達） | 高（merge kernel＋composition） | Opus 4.8 | High | Grok 4.5 | Sonnet 5 | 実行モデルOpus 4.8。terrain semantic checksumはproducer無しrequestで不変（直前tree実測）、blueprint／manifestの容器byteはplain module登録により変化（ADR 0038 D9） |
| V2-19-08 | 完了（2026-07-24） | Design-time support lint（reachable集合提示＋dispatch dry-run、report-only） | 中（通常実装） | Grok 4.5 | — | Sonnet 5 | — | 実行モデルOpus 4.8。design auditへ任意`supportLint`を追加（absentでbyte不変）、dispatch route・registry checksum・terrain semantic checksumは不変 |
| V2-19-09 | 完了（2026-07-24） | Coastal 4種必須の緩和ADR＋実装（部分集合許可条件・checksum影響） | 高（V2-2契約変更） | Opus 4.8 | High | Grok 4.5 | **ADR人間承認済み**（ADR 0040、D1 サイズ0〜4採択） | 実行モデルOpus 4.8。既存4種fixtureのtile semantic checksum `20318e6c…`は絶対pinで不変、容器byte・registry checksum・reachability projection・sealed catalogも不変。capability昇格なし |
| V2-19-10 | 完了（2026-07-24） | Material／palette block反映（resolver接続、11 state→profile allowlist、実field validation） | 中高（semantic checksum計画） | Sonnet 5 | High | Grok 4.5 | ADR承認範囲内（0038 D9、新ADR不要） | 実行モデルOpus 4.8。`environment-fields`のtile／block semantic checksumのみ変化（意図どおり）、`surface-2_5d`／`hydrology-plan`／ADR 0037 adapter・sealed catalog・dispatch registry・v1 goldenは不変 |
| V2-19-12 | 完了（2026-07-24） | Coherent detail kernel設計ADR＋実装（連続multi-scale detail、budget固定、erosion分離） | 高（新表現契約） | Opus 4.8 | High | Grok 4.5 | **ADR人間承認済み**（ADR 0041、D2 background base-level cellのみ／D3 request level optional採択） | 実行モデルOpus 4.8。`foundationDetail`絶対なしのterrain semantic checksum・容器byte・sealed catalog・dispatch registry・reachability projectionは不変、capability昇格なし |
| V2-19-13 | 完了（2026-07-24） | Blueprint拡張面積縮小のADR検討（typed plan envelope案、Proposed著述まで） | 中高（設計文書） | Opus 4.8 | High | Sonnet 5 | — | 実行モデルOpus 4.8。[ADR 0042](../adr/0042-blueprint-typed-plan-envelope.md) `Proposed`著述のみ、実装は採択後の新ID。production code・Schema・checksum不変 |
| V2-19-14 | 完了（2026-07-24） | mask⇔feature SOFT reconcile pre-pass ADR＋実装（決定論的snap、tolerance超拒否） | 高（HARD不発明原則との整合） | Opus 4.8 | High | Grok 4.5 | **ADR人間承認済み**（ADR 0043、D1 剛体平行移動／D3 request level optional採択） | 実行モデルOpus 4.8。`maskFeatureReconcile`絶対なしのterrain semantic checksum・容器byte・sealed catalog・dispatch registry・reachability projectionは不変、capability昇格なし |
| V2-19-15 | 完了（2026-07-24） | Commit message規約（Task ID必須等、docs-only） | 低 | Haiku 4.5 | — | Grok 4.5 | — | docs-only（`AGENTS.md` §16新設）。production code・checksum不変 |
| V2-19-16 | 完了（2026-07-24、**人間承認済み**） | Phase gate（全leaf再検証、materialization gate発効確認、full clean suite） | 最高（Phase横断） | **Fable 5** | Highest available | Grok 4.5 | Opus 4.8＋**人間承認** | 実行モデルFable 5（割当どおり）。gateは計測のみ、checksum変化なし・capability昇格なし。人間承認で親Phaseを閉じた |
