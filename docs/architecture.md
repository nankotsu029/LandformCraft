...
コードはWorldEdit 7.3.19の公開APIへcompileOnlyで依存します。FAWEが導入されている環境ではWorldEdit互換APIとして利用します。WorldEditとFAWEを同時に導入しません。Release 2のFAWE単独smoke profileは [ADR 0032](adr/0032-fawe-2.15.2-standalone-smoke-profile.md) が **FAWE 2.15.2** を固定します。`V2-6-14`／`V2-6-15`の現状は [re-audit](design-v2/audits/v2-6-14-v2-6-15-reaudit.md)（いずれも `BLOCKED_EXTERNAL`）を正本にします。0.9.0-beta.1のv1 Beta smoke結果はbeta auditとrelease checklistを正本にします。
...
この文書の1〜11節は `0.9.0-beta.1`／TerrainIntent v1／generator `3.0.0-phase6`／Release format 1の現行アーキテクチャです。[ADR 0010](adr/0010-adopt-versioned-terrain-v2-roadmap.md) は、これをin-placeで置換せず、version分離したv2を段階導入する方針を採用しました。2026-07-17の再設計（[ADR 0015](adr/0015-adopt-v2-foundation-three-track-roadmap.md)）はこのv2基盤を維持したまま、scale契約（`model.v2.scale`／`core.v2.scale`、[ADR 0016](adr/0016-scale-classes-and-execution-planning.md)）と決定論的画像抽出（`format.v2.constraint.extract`、[ADR 0017](adr/0017-deterministic-image-mask-extraction.md)）を`EXPERIMENTAL`で追加してroadmapを3トラックへ再構築し、2026-07-18にV2-9／V2-10をTrack D／Eとして独立させて5トラックへ拡張しました。進捗と次Taskは [roadmap](roadmap.md#terrain-generation-v2) を正本とします。V2-4はtyped geology／lithology／strata、coarse climate prior／final fields、regional water／snow、semantic material、1.21.11 palette adapter（[ADR 0018](adr/0018-minecraft-palette-adapter-v2.md)）、mangrove／coral、sparse ecology、volcanic／canyon material、validator／preview、strict Release 2 `environment-fields`（[ADR 0019](adr/0019-environment-fields-release-capability.md)）を統合し、Phase gateを完了しました。対象offline feature／capabilityは`SUPPORTED`ですが、V2-3 artifact、v1 palette／Release能力、公開CLI／Paperは変更していません。V2-5はsparse volume（SDF／CSG／index／cache／volume query、8 volume feature、local environment、validator／preview、offline 3D read-back、Release 2 `sparse-volume`）のPhase gateを完了し、offline `SUPPORTED`です。V2-6は`V2-6-13`まで完了し、Release 2のplan→envelope→reservation/confirm→snapshot-all→containment→canonical apply→bounded settle／full verify→rollback／Undo／Recovery、provider/manual/image design path、cross-capability hardening、運用metrics／diagnostics／retentionをversion分離して実装しました（ADR 0020〜0031）。terminal `APPLIED`はsettle後のeffect envelope exact verify後だけです。Release 2 public command接続、WorldEdit／FAWE実機smoke、production capabilityは未完了で、`V2-6-14`／`V2-6-15`は`BLOCKED_EXTERNAL`です。V2-9／V2-10はoffline plan-levelの各Phase gateまで完了していますが、Paper apply capabilityはV2-6 gate後の別Taskまで`UNSUPPORTED`です。
...
`WorldBlueprintV2`はdiagnostic用`DESCRIPTOR_ONLY`に加え、artifact metadataと一致する`SIDECAR` fieldを表せます。値はBlueprint JSONへ埋め込まず、row-major sidecarをbounded windowで読みます。V2-0では`TerrainQuery`／`TerrainBlockResolver`とv1 adapterが現行materializerの全block出力を再現します。V2-5は`VolumeTerrainQueryV2`（`terrain-query-volume-v1`）でbase heightfieldへordered CSGを合成し、volume featureのcanonical block streamをoffline 3D tile（`V2-5-16`）とRelease 2 `sparse-volume`（`V2-5-17`）まで接続したが、v1 adapter意味とRelease 1は不変で、Paperは未接続である。V2-1 manual pathはAIを呼びません。V2-2-03〜07はcoastal semantic fieldと明示transitionを生成し、V2-2-08は独立validator／preview、V2-2-09はfinal resolverからV2専用Sponge v3 tileへ二走査streamingし`worldedit.v2`でoffline read-backします。V2-2-10はempty-capability Release 2 coreを、V2-2-11はこれらをcomplete setとして収容するoffline `surface-2_5d` capabilityを`format.v2.release`へ並設しました。V2-3-14は`hydrology-plan`を`surface-2_5d`依存で追加し、V2-3-15でstrict capabilityと完成featureのoffline lifecycleを監査しました。Paper world変更は未実装で、v1のcodec、generator、Release publisher／verifier、配置／Undoへv2型は接続していません。
