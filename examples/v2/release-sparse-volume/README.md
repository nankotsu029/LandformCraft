...
`V2-15-08`では`Release2SparseVolumeExportApplicationServiceV2`と
`SparseVolumeExportPipelineV2`がこの既存layoutをproduction export APIへ接続した。現時点では個別volume
Featureを公開せず、release-local bedrock cellへ作用して結果を変えないordinal 0 `ADD_FLUID` operatorで
ordered CSGを実行する。SDF／CSG／AABB／validationのbindingとvolume tileのstrict Sponge read-backを通過した
directory／ZIPだけを公開する。Schema、artifact type／version、capability名、CLI既定、Paper supportは不変である。
