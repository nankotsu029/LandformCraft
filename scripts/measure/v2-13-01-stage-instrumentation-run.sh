#!/usr/bin/env bash
# V2-13-01 placement stage instrumentation + 1000×1000 baseline re-measurement.
#
# Same real-host methodology as V2-11-05 (standalone Paper JVM in run-fawe/, FAWE 2.15.2 only,
# WorldEdit plugin forbidden, measurement profile ceiling 1000×1000, RCON, PID-only telemetry).
# The ONLY additive behaviour is: this runner records a wall-clock epoch at each observable
# placement lifecycle transition (a "mark") and, after each pass, emits committed per-stage
# duration evidence via v2-13-01-stage-durations.py so the ~106 min/pass total is decomposed into
# PLAN / SNAPSHOT / APPLY / SETTLE / VERIFY / UNDO and the bottleneck stage is named.
#
# It changes NO safety ordering and NO verify scope. It is real-machine dependent: without a
# dedicated high-memory measurement host it cannot run (Task status BLOCKED_EXTERNAL). See
# docs/smoke/v2-13-01-stage-instrumentation-runbook.md.
set -euo pipefail
...
PROFILE="fawe"
MEASURE_ROOT="$ROOT/build/measure/v2-13-01"
...
RCON_PASSWORD="v21301-measure-local"
RCON_PORT=25576
GAME_PORT=25566
WORLD_NAME="world"
...
TELEMETRY_PID=""
PAPER_LAUNCH_PID=""
PLAN_WAIT_SEC="${LANDFORMCRAFT_V21301_PLAN_WAIT_SEC:-1800}"
EXEC_TIMEOUT_SEC="${LANDFORMCRAFT_V21301_EXEC_TIMEOUT_SEC:-10800}"
UNDO_TIMEOUT_SEC="${LANDFORMCRAFT_V21301_UNDO_TIMEOUT_SEC:-10800}"
...
# --- additive stage-mark capture -------------------------------------------------------------
MARKS_FILE=""
mark_init() { MARKS_FILE="$1"; : >"$MARKS_FILE"; }
record_mark() {
  # Idempotent per name: first observation wins (the emitter also takes last-write, but journal
  # states are monotonic so the first sighting is the true boundary).
  local name="$1"
...
echo "==> exporting 1000x1000 solid surface Release (shared V2-11-05 measurement fixture)"
rm -rf "$EXPORT_DIR"
...
# Reuse the V2-11-05 stone-seal + keep-load envelope helper verbatim (necessary for 1000×1000).
seal_and_forceload() {
  local life_dir="$1"
...
run_lifecycle() {
  ...
  # The confirmation file already holds the canonical "/landformcraft v2 place confirm <id> <token>".
  ...
  { echo "label=$label"; echo "placement_id=$placement_id";
    echo "wall_seconds=$((end_epoch - start_epoch))"; echo "dimensions=1000x1000"; } \
  ...
  python3 "$STAGE_EMITTER" --marks "$life_dir/marks.tsv" \
    --pass-id "$label" --dimensions 1000x1000 --profile "fawe-2.15.2" \
