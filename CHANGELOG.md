...
- V2-11-05 FAWE 1000×1000 Paper再実測 — **完了**（無効化済み`V2-6-17`の後継。`run-fawe/` paperclip単独JVM＋RCON＋PID telemetry、FAWE 2.15.2のみ／WorldEdit禁止、measurement profile ceiling 1000×1000。Pass1／Pass2とも`APPLIED`→full effect-envelope verify→`UNDONE`（wall ≈ 6342／6436 s、peak RSS ≈ 6.02 GiB、Xmx 8G／Xms 2G）。≤256-chunk strip forceload、journal `SNAPSHOT_COMPLETE`待ち、MEDIUM envelopeの`Map.copyOf` MapN欠陥を`Collections.unmodifiableMap`へ修正、disk admission超過なし、plan→SIGKILL recovery、v1 help／doctor。catalog寸法は64×64のまま。証拠: [audit](docs/design-v2/audits/v2-11-05-fawe-1000-measurement.md)／[evidence](docs/design-v2/audits/v2-11-05-fawe-1000-measurement-evidence.md)。次は`V2-11-06`）
- V2-11-04 FAWE 500×500 Paper再実測 — **完了**（無効化済み`V2-6-16`の後継。`run-fawe/` paperclip単独JVM＋RCON＋PID telemetry、FAWE 2.15.2のみ／WorldEdit禁止、measurement profile ceiling 500×500。Pass1／Pass2とも`APPLIED`→full effect-envelope verify→`UNDONE`（wall ≈ 621／627 s、peak RSS ≈ 6.08 GiB、Xmx 6G）。stone-seal＋block-column `forceload`、Pass間Paper再起動、plan→SIGKILL recovery、v1 help／doctor。catalog寸法は64×64のまま。証拠: [audit](docs/design-v2/audits/v2-11-04-fawe-500-measurement.md)／[evidence](docs/design-v2/audits/v2-11-04-fawe-500-measurement-evidence.md)。次は`V2-11-05`）
