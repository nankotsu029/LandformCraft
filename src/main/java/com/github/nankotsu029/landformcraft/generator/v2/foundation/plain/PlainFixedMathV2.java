...
/** Integer-only helpers for V2-9-02 plain polygon tests and deterministic sampling. */
public final class PlainFixedMathV2 {
    private PlainFixedMathV2() {
    }

    public static boolean contains(List<PlainPlanV2.Ring> rings, long x, long z) {
    ...
    static boolean insideOrBoundary(List<PlainPlanV2.Vertex> ring, long x, long z) {
        boolean inside = false;
        for (int index = 0; index < ring.size() - 1; index++) {
            PlainPlanV2.Vertex a = ring.get(index);
            PlainPlanV2.Vertex b = ring.get(index + 1);
    ...
    static long cross(PlainPlanV2.Vertex a, PlainPlanV2.Vertex b, long x, long z) {
