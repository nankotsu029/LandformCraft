...
/** Integer-only helpers for V2-9-05 marsh polygon tests and deterministic sampling. */
public final class MarshFixedMathV2 {
    private MarshFixedMathV2() {
    }

    public static boolean contains(List<MarshPlanV2.Ring> rings, long x, long z) {
    ...
    static boolean insideOrBoundary(List<MarshPlanV2.Vertex> ring, long x, long z) {
        boolean inside = false;
        for (int index = 0; index < ring.size() - 1; index++) {
            MarshPlanV2.Vertex a = ring.get(index);
            MarshPlanV2.Vertex b = ring.get(index + 1);
    ...
    static long cross(MarshPlanV2.Vertex a, MarshPlanV2.Vertex b, long x, long z) {
