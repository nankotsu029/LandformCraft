...
/** Integer-only helpers for V2-9-05 floodplain polygon tests and deterministic sampling. */
public final class FloodplainFixedMathV2 {
    private FloodplainFixedMathV2() {
    }

    public static boolean contains(List<FloodplainPlanV2.Ring> rings, long x, long z) {
    ...
    static boolean insideOrBoundary(List<FloodplainPlanV2.Vertex> ring, long x, long z) {
        boolean inside = false;
        for (int index = 0; index < ring.size() - 1; index++) {
            FloodplainPlanV2.Vertex a = ring.get(index);
            FloodplainPlanV2.Vertex b = ring.get(index + 1);
    ...
    static long cross(FloodplainPlanV2.Vertex a, FloodplainPlanV2.Vertex b, long x, long z) {
