...
/** Strict codec/read-back verifier for the non-Release V2-5-15 volume preview bundle. */
public final class VolumePreviewIndexCodecV2 {
    public static final String SCHEMA = "volume-preview-index-v2.schema.json";
    ...
    public VolumePreviewIndexV2 read(Path path) throws IOException {
        ...
        VolumePreviewIndexV2 index = mapper.treeToValue(tree, VolumePreviewIndexV2.class);
        return verifyChecksum(index);
    }

    public VolumePreviewIndexV2 readAndVerify(
            Path indexPath,
            Path previewRoot,
        ...
        if (!indexPath.getFileName().toString().equals(INDEX_FILE_NAME)) {
            throw new IOException("volume preview index filename must be index.json");
        }
        ...
        if (Files.isSymbolicLink(indexPath) || !Files.isRegularFile(indexPath, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException("volume preview index must be a regular non-symbolic file");
        }
        VolumePreviewIndexV2 index = read(indexPath);
        ...
        for (int indexValue = 0; indexValue < index.layers().size(); indexValue++) {
            cancellationToken.throwIfCancellationRequested();
            VolumePreviewIndexV2.Layer layer = index.layers().get(indexValue);
            LoadedConstraintMapSource source = loaded.get(indexValue);
            if (source.sourceBytes() != layer.byteLength()) {
                throw new IOException("volume preview byte length mismatch: " + layer.path());
            }
    ...
    public VolumePreviewIndexV2 seal(VolumePreviewIndexV2 index) {
        ...
        if (!checksum.equals(index.canonicalChecksum())) {
            throw new IllegalArgumentException("volume preview index canonical checksum mismatch");
        }
        return index;
    }

    public void write(Path path, VolumePreviewIndexV2 index) throws IOException {
        VolumePreviewIndexV2 sealed = seal(index);
        ObjectNode tree = mapper.valueToTree(sealed);
        validator.validate(SCHEMA, path.toString(), tree);
        Path target = path.toAbsolutePath().normalize();
        Path parent = Objects.requireNonNull(target.getParent(), "volume preview index must have a parent");
        Files.createDirectories(parent);
        Path temporary = Files.createTempFile(parent, ".volume-preview-index-", ".tmp");
        boolean published = false;
        try {
            Files.write(temporary, CanonicalJsonV2.bytes(tree), StandardOpenOption.TRUNCATE_EXISTING);
            if (!read(temporary).equals(sealed)) {
                throw new IOException("volume preview index changed during staged read-back");
            }
            try {
                Files.move(temporary, target, StandardCopyOption.ATOMIC_MOVE, StandardCopyOption.REPLACE_EXISTING);
            } catch (AtomicMoveNotSupportedException exception) {
                throw new IOException("atomic move is required for volume preview index", exception);
            }
            published = true;
        } finally {
            if (!published) {
    ...
    private VolumePreviewIndexV2 verifyChecksum(VolumePreviewIndexV2 index) throws IOException {
        if (index.hasPendingCanonicalChecksum()) {
            throw new IOException("volume preview index is not canonically sealed");
        }
        if (!canonicalChecksum(index).equals(index.canonicalChecksum())) {
            throw new IOException("volume preview index canonical checksum mismatch");
        }
        return index;
    }

    private String canonicalChecksum(VolumePreviewIndexV2 index) {
    ...
    private static void requireSafeDirectory(Path root) throws IOException {
        if (Files.isSymbolicLink(root) || !Files.isDirectory(root, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException("volume preview root must be a non-symbolic directory");
        }
    }

    private static void verifyDirectoryEntries(Path root, VolumePreviewIndexV2 index) throws IOException {
        Set<String> expected = new HashSet<>(index.layers().stream().map(VolumePreviewIndexV2.Layer::path).toList());
        expected.add(INDEX_FILE_NAME);
        try (var paths = Files.list(root)) {
            Set<String> actual = new HashSet<>();
            for (Path path : paths.toList()) {
                if (Files.isSymbolicLink(path) || !Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)) {
                    throw new IOException("volume preview root contains an unsafe entry");
                }
                actual.add(path.getFileName().toString());
            }
            if (!actual.equals(expected)) {
                throw new IOException("volume preview index and directory entries differ");
            }
        }
    }

    private static void verifyPng(byte[] bytes, VolumePreviewIndexV2.Layer layer) throws IOException {
        try (ImageInputStream input = ImageIO.createImageInputStream(new ByteArrayInputStream(bytes))) {
            if (input == null) {
                throw new IOException("volume preview image input is unavailable");
            }
            var readers = ImageIO.getImageReaders(input);
            if (!readers.hasNext()) {
                throw new IOException("volume preview is not a supported PNG");
            }
            ImageReader reader = readers.next();
            try {
                reader.setInput(input, false, true);
                if (reader.getNumImages(true) != 1) {
                    throw new IOException("volume preview must have one frame");
                }
                int width = reader.getWidth(0);
                int length = reader.getHeight(0);
                if (width != layer.width() || length != layer.length()
                        || width > 256 || length > 256
                        || (long) width * length > 65_536L) {
                    throw new IOException("volume preview PNG dimensions exceed its index budget: "
                            + layer.path());
                }
                BufferedImage image = reader.read(0);
                if (image == null || image.getWidth() != width || image.getHeight() != length) {
                    throw new IOException("volume preview PNG decode failed: " + layer.path());
                }
                image.flush();
            } finally {
                reader.dispose();
