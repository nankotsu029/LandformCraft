...
/**
 * Thrown by {@link CanyonBedMaterializationV2} when a {@code CANYON} cross-section cannot be
 * materialized into the final canonical block stream without breaking one of the declared
 * interaction contracts (V2-15-12).
 *
 ...
 */
public final class CanyonMaterializationRejectedV2 extends IOException {
    ...
    CanyonMaterializationRejectedV2(String ruleId, String featureId, int cellX, int cellZ, String detail) {
    ...
    /** Offending release-local cell, or {@code -1} for a whole-corridor rejection. */
    public int cellX() {
        return cellX;
    }
    ...
    private static String message(String ruleId, String featureId, int cellX, int cellZ, String detail) {
        String cell = cellX < 0 || cellZ < 0 ? "corridor" : cellX + "," + cellZ;
        return Objects.requireNonNull(ruleId, "ruleId")
                + " [" + CanyonBedMaterializationV2.CONTRACT_VERSION + "] canyon "
