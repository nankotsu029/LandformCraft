...
/**
 * Thrown by {@link RiverBedMaterializationV2} when a {@code RIVER} / {@code MEANDERING_RIVER} route
 * cannot be materialized into the final canonical block stream without breaking one of the declared
 * interaction contracts (V2-19-05).
 *
 ...
 */
public final class RiverMaterializationRejectedV2 extends IOException {
    ...
    RiverMaterializationRejectedV2(String ruleId, String featureId, int cellX, int cellZ, String detail) {
    ...
    /** Offending release-local cell, or {@code -1} for a whole-route rejection. */
    public int cellX() {
        return cellX;
    }
    ...
    private static String message(String ruleId, String featureId, int cellX, int cellZ, String detail) {
        String cell = cellX < 0 || cellZ < 0 ? "route" : cellX + "," + cellZ;
        return Objects.requireNonNull(ruleId, "ruleId")
                + " [" + RiverBedMaterializationV2.CONTRACT_VERSION + "] river "
